        ...
        if (response.success) {
          ...
          if (desigList.length > 0) {
            const orgDesigList = desigList.map(item => {
              const desigObj = this.commonObj.designationList.find(desig => desig.value === parseInt(item.designation_id))
      ...
      RestApi.getData(commonServiceBaseUrl, 'common-dropdowns', null).then(response => {
        if (response.success) {
          this.$store.dispatch('CommonService/mutateCommonObj', {
            ...
            portalServiceNameList: response.data.portalServiceNameList
          })
