  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exportTrophyCircular.circular_approve') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophyCircular.circular_approve') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button v-b-modal.modal-form-publish variant="iq-bg-success mr-1" size="sm" v-if="data.item.status === 3" @click="publish(data.item)" class="action-btn active" :title="$t('globalTrans.publish')"><i class="ri-check-fill"></i></b-button>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-form-details" size="lg" :title="forwardTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Details :id="editItemId" :key="editItemId" :userDetails="userDetails"/>
    </b-modal>
    <b-modal id="modal-form-publish" size="lg" :title="approveTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Publish :id="editItemId" :key="editItemId"/>
    </b-modal>
  </div>
</template>
<style scoped>
...
import { circularApproveListApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, circularApproveListApi, params)
      if (result.success) {
