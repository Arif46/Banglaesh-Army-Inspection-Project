  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exportTrophyCircular.circular_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophyCircular.circular_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <slot v-if="data.item.last_forward.receiver_id === loggedUserId">
                      <b-button v-b-modal.modal-form-forward variant="iq-bg-success mr-1" size="sm" @click="forward(data.item)" class="action-btn active" :title="$t('globalTrans.forward')"><i class="ri-send-plane-fill"></i></b-button>
                      <b-button v-b-modal.modal-form-approve variant="iq-bg-success mr-1" size="sm" v-if="data.item.status === 2" @click="approve(data.item)" class="action-btn active" :title="$t('globalTrans.approve')"><i class="ri-check-fill"></i></b-button>
                    </slot>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-form-approve" size="lg" :title="approveTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Approve :id="editItemId" :key="editItemId"/>
    </b-modal>
  </div>
</template>
<style scoped>
...
import { circularRequestListApi, circularToggleApi } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('exportTrophyCircular.circular') + ' ' + this.$t('globalTrans.entry') : this.$t('exportTrophyConfig.awardType') + ' ' + this.$t('globalTrans.modify')
    },
    detailsTitle () {
      return this.$t('exportTrophyCircular.circular') + ' ' + this.$t('globalTrans.approve')
    },
    forwardTitle () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, circularRequestListApi, params)
      if (result.success) {
