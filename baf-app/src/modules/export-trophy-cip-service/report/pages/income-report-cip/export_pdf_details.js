...
const exportPdfDetails = async (baseUrl, uri, orgId, reportTitle, vm, newDatas, search, searchHeader) => {
    ...
    const allRowD = []
      const firstRowHeader = [
        { text: vm.$t('globalTrans.fiscal_year') + ' : ' + (search.fiscal_year_id ? (i18n.locale === 'en' ? searchHeader.fiscal_year_name : searchHeader.fiscal_year_name_bn) : vm.$t('globalTrans.all')), alignment: 'left', style: 'th', bold: true },
