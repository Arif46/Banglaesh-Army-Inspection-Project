  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('cip.cabinet_committee_approval') }}</h4>
      </template>
      <template v-slot:headerAction>
        <b-overlay :show="loading">
          <div v-if="headerActionShow">
            ...
            <a href="javascript:" class="btn-add" @click="showModal"> {{ !showForwardBtn ? $t('cip.sorting_send') : $t('cip.forward') }}</a>
          </div>
        </b-overlay>
      </template>
    ...
    <b-modal id="modal-remark" size="md" :title="$t('globalTrans.remark')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Remark :circular_id="circularId" :current_status="13" :status="14" :menu_name_id="4" :key="circularId"/>
    </b-modal>
...
import { commApprovalListApi, commApprovReqVerifiedAppsApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, commApprovalListApi, this.search)
      this.loading = false
      if (result.success) {
        this.headerActionShow = true
        this.getRelationalData(result.data.data)
    ...
    async downloadCipApplicants () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, commApprovReqVerifiedAppsApi, params)
      this.loading = false
      if (result.success) {
