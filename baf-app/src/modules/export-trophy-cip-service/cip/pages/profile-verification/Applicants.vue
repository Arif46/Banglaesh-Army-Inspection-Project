<template>
  <div class="inner-section">
            ...
            <b-col md="12" class="table-responsive">
              <b-table thead-class="table_head" striped bordered hover :items="listData" :fields="columns" aria-hidden="loading | listReload ? 'true' : null" :emptyText="$t('globalTrans.noDataFound')" show-empty>
                ...
                <template v-slot:cell(action)="data">
                  ...
                  <slot v-if="data.item.status === 9">
                    <b-button v-b-modal.modal-verify variant=" action-btn status mr-1" size="sm" @click="singleData(data.item)" class="action-btn active" :title="$t('globalTrans.verify')"><i class="ri-message-2-line"></i></b-button>
    ...
    <b-modal id="modal-details" size="xl" :title="$t('globalTrans.details')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
     ...
     <b-modal id="modal-verify" size="md" :title=" $t('globalTrans.verify')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Verify :id="editItemId" :key="editItemId"/>
    </b-modal>
  </div>
</template>
<script>
...
import { profileVerficationApplicantsApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      const params = { product_service_sector_id: this.$route.query.id, receiver_id: this.$store.state.Auth.authUser.user_id }
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, profileVerficationApplicantsApi, params)
      if (result.success) {
