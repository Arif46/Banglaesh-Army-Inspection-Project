      ...
      <b-form @submit.prevent="handleSubmit(submit)" @reset.prevent="reset" autocomplete="off">
        ...
        <b-row class="text-right">
          <b-col>
            ...
            <b-button variant="danger" class="btn-sm" @click="$bvModal.hide('modal-verify')">{{ $t('globalTrans.cancel') }}</b-button>
          </b-col>
        </b-row>
      </b-form>
    </b-overlay>
...
import { profileVerficationVerifyApi } from '../../api/routes'
export default {
  name: 'Verify',
  props: ['id'],
  data () {
  ...
  methods: {
    ...
    async submit () {
      ...
      const formData = Object.assign(this.form, { app_id: this.id, user_id: this.authUser.user_id })
      result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, profileVerficationVerifyApi, formData)
      ...
      if (result.success) {
        ...
        this.$bvModal.hide('modal-verify')
      } else {
