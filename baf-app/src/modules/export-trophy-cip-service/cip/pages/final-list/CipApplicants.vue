...
import { finalListApplicantsApi } from '../../api/routes'
...
export default {
  ...
  computed: {
    columns () {
      const labels = [
        ...
        { label: this.$t('globalTrans.year'), class: 'text-center' },
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, finalListApplicantsApi, params)
      if (result.success) {
        this.$emit('currentStatus', result.current_applications_status)
