  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('cip.approval_request') }}</h4>
      </template>
      <template v-slot:headerAction>
        <b-overlay :show="loading">
          <div v-if="headerActionShow">
            <a href="javascript:" v-if="$store.state.Auth.authUser.ssoLogin && showForwardBtn" class="btn-add status mr-2" @click="sendToNothi()" :title="$t('cip.send_to_nothi')">{{ $t('cip.send_to_nothi') }}</a>
            <a href="javascript:" class="btn-add mr-2" @click="downloadCipApplicants"> {{ $t('cip.all_verification') }}</a>
            <a href="javascript:" class="btn-add mr-1"  v-b-modal.modal-forward-comments :title="$t('globalTrans.comments')">{{ $t('cip.forward_comments') }}</a>
            <a href="javascript:" class="btn-add mr-1" v-b-modal.modal-forward> {{ $t('cip.forward') }}</a>
            <a href="javascript:" class="btn-add" v-b-modal.modal-handover> {{ approvalTitle }}</a>
          </div>
        </b-overlay>
      </template>
    ...
    <b-modal id="modal-forward-comments" size="lg" :title="$t('cip.forward_comments')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <FowardComments />
    </b-modal>
    <b-modal id="modal-forward" size="lg" :title="$t('cip.forward')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Forward />
    </b-modal>
    <b-modal id="modal-handover" size="md" :title="approvalTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { approvalReqListApi, approvalReqVerifiedAppsApi } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      loading: false,
      cabinetApprove: 0,
      approvalTitle: this.$t('cip.approval_for_committee_handover'),
      handoverTo: '',
      comments: [],
      comp: 'CipApplicants',
      year: '',
  ...
  computed: {
    ...
    pendingForward: function () {
      return this.$store.state.ExportTrophyCipService.commonObj.pendingForward
    },
  ...
  watch: {
    ...
    currentLocale: function (newVal, oldVal) {
      if (newVal !== oldVal) {
        this.checkCabinetOrPmo(this.cabinetApprove)
      }
    },
  ...
  methods: {
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, approvalReqListApi, this.search)
      this.loading = false
      this.cabinetApprove = result.cabinetApprove
      this.checkCabinetOrPmo(result.cabinetApprove)
      if (result.success) {
    ...
    checkCabinetOrPmo (cabinetApproveId) {
      if (cabinetApproveId === 0) {
        this.approvalTitle = this.$t('cip.approval_for_committee_handover')
        this.handoverTo = 1 // this.$t('cip.cabinet')
      } else {
        this.approvalTitle = this.$t('cip.approval_for_pmo_handover')
        this.handoverTo = 2 // this.$t('cip.pmo')
      }
    },
    async downloadCipApplicants () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, approvalReqVerifiedAppsApi, params)
      this.loading = false
      if (result.success) {
