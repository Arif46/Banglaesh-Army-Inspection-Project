  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('cip.mha_verified') }} {{ $t('globalTrans.list') }}</h4>
      </template>
                    ...
                    <table class="table table-hover table-bordered">
                        <thead>
                            <tr>
                                ...
                                <th style="width:17%">{{ $t('cip.sb') }}</th>
                                <th style="width:17%">{{ $t('cip.nsi') }}</th>
                                <th style="width:17%">{{ $t('globalTrans.action') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            <template v-for="(tmp, indx) in applications">
                                ...
                                <tr v-for="(item, index) in tmp.details" :key="index+indx+1">
                                    ...
                                    <td>
                                        ...
                                        {{ currentLocale === 'bn' ? item.designation_bn : item.designation_en }} <br/>
                                        {{ $t('cip.father') }}- {{ currentLocale === 'bn' ? item.father_name_bn : item.father_name_en }} <br/>
                                        {{ $t('cip.mother') }}- {{ currentLocale === 'bn' ? item.mother_name_bn : item.mother_name_en }} <br/>
                                    ...
                                    <td>
                                        ...
                                        {{ $n(item.office_contact_info.telephone_no, { useGrouping: false }) }}, {{ $n(item.office_contact_info.fax_no, { useGrouping: false }) }}
                                    </td>
                                    <template v-if="item.profile_verification && item.profile_verification.moha_sb">
                                      <td>{{ item.profile_verification.moha_sb }}</td>
                                      <td>{{ item.profile_verification.moha_nsi }}</td>
                                    </template>
                                    <template v-else>
                                      <td>
                                        <ValidationProvider name="SB Comment" :vid="`moha_sb${index+indx+1}`" rules="required|max:250">
                                            <b-form-group
                                                slot-scope="{ valid, errors }"
                                                :label-for="`moha_sb${index+indx+1}`"
                                            >
                                                <b-form-textarea
                                                    v-model="item.moha_sb"
                                                    :id="`moha_sb${index+indx+1}`"
                                      ...
                                      <td>
                                        <ValidationProvider name="NSI Comment" :vid="`moha_nsi${index+indx+1}`" rules="required|max:250">
                                            <b-form-group
                                                slot-scope="{ valid, errors }"
                                                :label-for="`moha_nsi${index+indx+1}`"
                                            >
                                                <b-form-textarea
                                                    v-model="item.moha_nsi"
                                                    :id="`moha_nsi${index+indx+1}`"
                                                    rows="4"
                                                    :placeholder="$t('cip.enter_your_comment')"
                                                    max-rows="6"
                                                    :state="errors[0] ? false : (valid ? true : null)"
...
import { mohaApplicationList, mohaVerify } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, mohaApplicationList)
      if (result.success) {
        this.applications = this.getRelationalData(result.data)
        const existVerfication = result.data[0].details[0].profile_verification
        if (existVerfication && existVerfication.moha_sb) {
    ...
    async submit () {
      ...
      result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, mohaVerify, formData)
      this.loading = false
    ...
    generateFormData (data) {
        ...
        data.map(tmp => {
            return tmp.details.map((item, index) => {
                const detailObj = {
                    app_id: item.id,
                    moha_user_id: this.authUser.user_id,
                    moha_sb: item.moha_sb,
                    moha_nsi: item.moha_nsi
                }
