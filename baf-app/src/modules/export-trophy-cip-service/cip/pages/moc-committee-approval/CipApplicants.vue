<template>
  <div class="inner-section">
            ...
            <b-col md="12" class="table-responsive">
              <b-table thead-class="table_head" striped bordered hover :items="listData" :fields="columns" aria-hidden="loading | listReload ? 'true' : null" :emptyText="$t('globalTrans.noDataFound')" show-empty>
                ...
                <template v-slot:cell(action)="data">
                  <!-- <b-button v-b-modal.modal-comments variant=" action-btn status mr-1" size="sm" @click="singleData(data.item)" class="action-btn status" :title="$t('globalTrans.forward') + ' ' + $t('globalTrans.comments')"><i class="ri-chat-2-line"></i></b-button> -->
                  ...
                  <b-button v-if="data.item.status === 10" v-b-modal.modal-approve-reject variant=" action-btn delete mr-1" size="sm" @click="approveReject(data.item, 3)" class="action-btn edit" :title="$t('globalTrans.reject')"><i class="ri-close-fill"></i></b-button>
                </template>
              </b-table>
              <b-pagination
                class="mt-20"
    ...
    <!-- <b-modal id="modal-comments" size="lg" :title="$t('globalTrans.forward') + ' ' + $t('globalTrans.comments')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { mocCommitteeApprovalApplicantsApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, mocCommitteeApprovalApplicantsApi, params)
      if (result.success) {
