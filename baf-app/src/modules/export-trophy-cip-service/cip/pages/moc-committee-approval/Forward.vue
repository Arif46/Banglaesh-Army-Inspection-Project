...
import { mocCommitteeApprovalForwardApi, designationWiseUser } from '../../api/routes'
...
export default {
  ...
  computed: {
    officeList () {
      return this.$store.state.CommonService.commonObj.officeList.filter(el => el.org_id === 1) // MOC
    },
    currentLocale () {
  ...
  methods: {
    ...
    async submit () {
      ...
      result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, mocCommitteeApprovalForwardApi, this.form)
      this.loading = false
