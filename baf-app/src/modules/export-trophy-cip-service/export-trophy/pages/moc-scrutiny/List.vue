<template>
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerAction>
        <b-overlay :show="loading">
          <a href="javascript:" v-if="$store.state.Auth.authUser.ssoLogin" variant="btn btn-primary" class="btn-add mr-2" @click="sendToNothi"> {{ $t('cip.send_to_nothi') }}</a>
    ...
    <b-modal id="modal-remark" size="md" :title="$t('globalTrans.remark')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Remark :circular_id="circularId" :current_status="8" :status="9" :menu_name_id="3" :key="circularId"/>
    </b-modal>
...
import { mocScrutinyListApi, previousMeetingResolutionApi, mocScrutVerifiedAppsApi, mocScrutinyNothiSend, mocScrutSendToMocCommitteApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    showApplicant (productServiceId) {
      this.$router.push({
        path: `/export-trophy-cip-service/export-trophy/moc-scrutiny-applicants?id=${productServiceId}`
      })
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, mocScrutinyListApi, params)
      if (result.success) {
    ...
    async downloadETApplicants () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, mocScrutVerifiedAppsApi, params)
      this.loading = false
      if (result.success) {
    ...
    sendToNothi () {
      ...
      const params = { divisionList: divisionList, districtList: districtList, cityCorporationList: cityCorporationList, upazilaList: upazilaList, municipalityList: municipalityList, countryList: countryList, unionList: unionList, wardList: wardList }
