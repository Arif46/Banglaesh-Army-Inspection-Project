  ...
  <div class="inner-section">
    <card>
        <template v-slot:searchHeaderTitle>
            <h4 class="card-title">{{ $t('cip.final_list') }} {{ $t('globalTrans.search') }}</h4>
        </template>
...
import { finalListApplicantstApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      const params = Object.assign({}, this.search, { product_service_sector_id: this.$route.query.id, fiscalyear: this.$route.query.fiscal_year })
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, finalListApplicantstApi, params)
      if (result.success) {
