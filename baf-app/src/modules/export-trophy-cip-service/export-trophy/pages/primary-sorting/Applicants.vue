            ...
            <b-col md="12" class="table-responsive">
              <b-table thead-class="table_head" striped bordered hover :items="listData" :fields="columns" aria-hidden="loading | listReload ? 'true' : null" :emptyText="$t('globalTrans.noDataFound')" show-empty>
                ...
                <template v-slot:cell(action)="data">
                  ...
                  <b-button v-if="data.item.status === 2" v-b-modal.modal-approve-reject variant=" action-btn status mr-1" size="sm" @click="approveReject(data.item, 4)" class="action-btn active" :title="$t('cip.primary_select')"><i class="ri-check-line"></i></b-button>
                  <b-button v-if="data.item.status === 2" v-b-modal.modal-approve-reject variant=" action-btn delete mr-1" size="sm" @click="approveReject(data.item, 3)" class="action-btn edit" :title="$t('globalTrans.reject')"><i class="ri-close-fill"></i></b-button>
                </template>
              </b-table>
...
export default {
  ...
  methods: {
    ...
    approveReject (item, status) {
      ...
      this.item = {
        status: status,
        note_title: status === 3 ? this.$t('globalTrans.reject') : this.$t('cip.primary_select')
      }
    },
    async loadData () {
