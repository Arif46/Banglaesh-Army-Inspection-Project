  ...
  try {
    ...
    const allRows = [
      [
        ...
        { text: vm.$t('cip.sb'), style: 'th', alignment: 'center', bold: true },
        { text: vm.$t('cip.nsi'), style: 'th', alignment: 'center', bold: true }
      ]
    ]
    ...
    vm.applications.forEach((item, index) => {
      if (item.sector_id) {
        ...
        allRows.push([
          { text: vm.$t('exportTrophy.product_service_sector') + ': ' + productServiceSectorName, rowSpan: item.details.length, colSpan: 6, alignment: 'center', style: 'td' },
          {},
          {},
          {},
          {},
      ...
      item.details.forEach((data, index1) => {
        ...
        const mother = vm.$t('cip.mother') + '- ' + (i18n.locale === 'en' ? data.owner_info.mother_name_en : data.mother_name_bn) + '\n'
        ...
        const orgNameAddress = orgName + orgAddress
        ...
        allRows.push([
          ...
          { text: (data.profile_verification && data.profile_verification.moha_sb) ? vm.$n(data.profile_verification.moha_sb) : '', alignment: 'center', style: 'td' },
          { text: (data.profile_verification && data.profile_verification.moha_nsi) ? vm.$n(data.profile_verification.moha_nsi) : '', alignment: 'center', style: 'td' }
        ])
      })
    })
    pdfContent.push({
