  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('cip.profile_verification') }}</h4>
      </template>
            ...
            <b-col md="12" class="table-responsive">
              <b-table thead-class="table_head" striped bordered hover :items="listData" :fields="columns" aria-hidden="loading | listReload ? 'true' : null" :emptyText="$t('globalTrans.noDataFound')" show-empty>
                ...
                <template v-slot:cell(action)="data">
                  <b-button v-b-modal.modal-verification-notes variant=" action-btn status mr-1" size="sm" @click="singleData(data.item)" class="action-btn status" :title="$t('cip.verification_note')"><i class="ri-booklet-line"></i></b-button>
                  ...
                  <b-button v-if="data.item.status === 11" v-b-modal.modal-approve-reject variant=" action-btn status mr-1" size="sm" @click="approveReject(data.item, 12)" class="action-btn active" :title="$t('globalTrans.approve')"><i class="ri-check-line"></i></b-button>
                  <b-button v-if="data.item.status === 11" v-b-modal.modal-approve-reject variant=" action-btn delete mr-1" size="sm" @click="approveReject(data.item, 3)" class="action-btn edit" :title="$t('globalTrans.reject')"><i class="ri-close-fill"></i></b-button>
                </template>
              </b-table>
