        ...
        <b-form @submit.prevent="handleSubmit(submit)" @reset.prevent="reset" autocomplete="off">
            <b-row>
                <b-col lg="12" md="12" sm="12">
                    ...
                    <ValidationProvider name="Export Trophy List" vid="export_trophy_list">
                        <b-form-group
                            class="row"
                            label-cols-sm="4"
                            label-for="export_trophy_list"
                        >
                            <template v-slot:label>
                                {{ $t('exportTrophy.export_trophy_list') }} :
                            </template>
                            <b-button variant="success" class="btn-xs mr-2" @click="downloadETList" :title="$t('globalTrans.download')"><i class="ri-download-cloud-fill"></i></b-button>
                        </b-form-group>
                    </ValidationProvider>
...
import { handoverApplicantstApi, pmoHandoverApi } from '../../api/routes'
import moment from 'moment'
import ExportPdf from './export_trophy_pdf'
...
export default {
    ...
    methods: {
        async submit () {
            this.loading = true
            const formData = Object.assign(this.form, { status: this.currentAppStatus })
            this.$store.dispatch('mutateCommonProperties', { loading: true })
            let result = null
            const loadingState = { loading: false, listReload: false }
            result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, pmoHandoverApi, formData)
            this.loading = false
        ...
        async downloadETList () {
            this.loading = true
            this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
            const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, handoverApplicantstApi, this.search)
            this.loading = false
            if (result.success) {
        ...
        pdfExport () {
            const reportTitle = this.$t('exportTrophy.pmo_approval_app_list')
