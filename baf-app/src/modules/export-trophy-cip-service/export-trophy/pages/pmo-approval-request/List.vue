  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophy.pmo_approval_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
      <template v-slot:headerAction v-if="showSendBtn">
        <b-overlay :show="loading">
          ...
          <a href="javascript:" class="btn-add" v-b-modal.modal-handover>{{ $t('cip.handover') }}</a>
        </b-overlay>
      </template>
    ...
    <b-modal id="modal-handover" size="md" :title="$t('cip.forward_handover')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Handover />
    </b-modal>
...
import { pmoApprovalReqListApi, profileVerificationSendApi, pmoApprovalReqVerifiedAppsApi } from '../../api/routes'
...
export default {
  ...
  components: {
    MeetingResolution, Handover, Details
  },
  data () {
    return {
  ...
  methods: {
    showApplicant (productServiceId) {
      this.$router.push({
        path: `/export-trophy-cip-service/export-trophy/pmo-approval-request-applicants?id=${productServiceId}`
      })
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, pmoApprovalReqListApi, params)
      if (result.success) {
    ...
    async send () {
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, profileVerificationSendApi)
      if (result.success) {
        this.$toast.success({
    ...
    async downloadETApplicants () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, pmoApprovalReqVerifiedAppsApi, params)
      this.loading = false
      if (result.success) {
