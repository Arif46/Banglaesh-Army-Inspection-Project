            ...
            <b-col md="12" class="table-responsive">
              <b-table thead-class="table_head" striped bordered hover :items="listData" :fields="columns" aria-hidden="loading | listReload ? 'true' : null" :emptyText="$t('globalTrans.noDataFound')" show-empty>
                ...
                <template v-slot:cell(action)="data">
                  ...
                  <slot v-if="data.item.status === 10">
                    <b-button v-b-modal.modal-approve-reject variant=" action-btn status mr-1" size="sm" @click="approveReject(data.item, 11)" class="action-btn active" :title="$t('globalTrans.approve')"><i class="ri-check-line"></i></b-button>
...
import { committeeApprovalApplicantstApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, committeeApprovalApplicantstApi, params)
      if (result.success) {
