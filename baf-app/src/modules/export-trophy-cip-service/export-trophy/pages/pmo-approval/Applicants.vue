  ...
  <div class="inner-section">
    <card>
        <template v-slot:searchHeaderTitle>
            <h4 class="card-title">{{ $t('cip.sorting_approve') }} {{ $t('globalTrans.search') }}</h4>
        </template>
...
import { pmoApprovalApplicantstApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, pmoApprovalApplicantstApi, params)
      if (result.success) {
