  ...
  try {
    ...
    vm.applications.forEach((item, index) => {
      ...
      item.details.forEach((data, index1) => {
        ...
        const father = data.owner_info ? (vm.$t('cip.father') + '- ' + (i18n.locale === 'en' ? data.owner_info.father_husband_name_en : data.owner_info.father_husband_name_bn)) : '' + '\n'
        const mother = data.owner_info ? (vm.$t('cip.mother') + '- ' + (i18n.locale === 'en' ? data.owner_info.mother_name_en : data.owner_info.mother_name_bn)) : '' + '\n'
        const nameAddress = name + designation + father + mother + getAddress(data.address)

        const orgName = data.org_info ? (i18n.locale === 'en' ? data.org_info.org_name_en : data.org_info.org_name_bn) : '' + '\n'
