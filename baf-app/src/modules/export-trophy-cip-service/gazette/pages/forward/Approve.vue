<template>
  <ValidationObserver ref="form" v-slot="{ handleSubmit, reset }">
    ...
    <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off">
      <b-row>
        <b-col sm="12">
          <h5 class="card-title"><b>{{ $t('exportTrophyGazette.gazette') }} : {{ currentLocale === 'bn' ? form.title_bn : form.title_en }}</b></h5>
        </b-col>
        <b-col sm="12">
...
import { gazetteApproveApi } from '../../api/routes'
export default {
  name: 'Form',
  props: ['id'],
  data () {
  ...
  methods: {
    ...
    async saveUpdate () {
      ...
      result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, gazetteApproveApi, this.form)
