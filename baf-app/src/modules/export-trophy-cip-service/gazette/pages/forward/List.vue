  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title"> {{ $t('exportTrophyGazette.gazette_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophyGazette.gazette_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <slot v-if="data.item.last_forward.receiver_id === loggedUserId">
                      <b-button v-if="data.item.status === 2" v-b-modal.modal-form-forward variant="iq-bg-success mr-1" size="sm" @click="forward(data.item)" class="action-btn active" :title="$t('globalTrans.forward')"><i class="ri-send-plane-fill"></i></b-button>
                      <b-button v-if="data.item.status === 2" v-b-modal.modal-form-approve variant="iq-bg-success mr-1" size="sm" @click="approve(data.item)" class="action-btn status" :title="$t('globalTrans.approve')"><i class="ri-check-fill"></i></b-button>
                    </slot>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-form-approve" size="lg" :title="approveDetails" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { gazetteRequestListApi, circularToggleApi } from '../../api/routes'
...
export default {
  ...
  components: {
    Form, Details, Forward, Approve, ForwardDetails
  },
  data () {
    return {
      sortBy: '',
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, gazetteRequestListApi, params)
      if (result.success) {
