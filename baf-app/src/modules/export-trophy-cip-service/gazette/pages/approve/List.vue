  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('globalTrans.approve') }} {{ $t('exportTrophyGazette.gazette') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('globalTrans.approve') }} {{ $t('exportTrophyGazette.gazette') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal id="modal-form-publish" size="lg" :title="publishedDetails" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { gazetteApproveListApi, circularToggleApi } from '../../api/routes'
...
export default {
  ...
  components: {
    Details, Publish, ForwardDetails
  },
  data () {
    return {
      sortBy: '',
  ...
  computed: {
    ...
    publishedDetails () {
      return this.$t('globalTrans.publish')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, gazetteApproveListApi, params)
      if (result.success) {
