      ...
      if (i18n.locale === 'bn') {
        ...
        const allRows = [
          [
            ...
            { image: vm.logo.right_logo_pdf, height: 70, width: 70, alignment: 'center' },
        ...
        const rows = [
          [
            { text: vm.$t('exportTrophyCircular.memo_no') + ' : ' + vm.form.memo_no, style: 'tdCustom3', alignment: 'left', rowSpan: 2 },
            { text: vm.$t('globalTrans.date') + ':', style: 'tdCustom3', alignment: 'left', rowSpan: 2 },
        ...
        if (vm.form.type === 1) {
          pdfContent.push({ text: vm.currentLocale === 'bn' ? 'তারিখ: ' + vm.getBnDateLocals + '/' + vm.getDateLocals : 'Date: ' + vm.getBnDateLocals + '/' + vm.getDateLocals, style: 'subHeaderFont', alignment: 'center' })
        }
        if (vm.form.type === 2) {
        ...
        if (vm.form.type === 1) {
          pdfContent.push({ text: vm.form.memo_no + ' ' + getTitle(), style: 'td', alignment: 'left' })
        ...
        if (vm.form.type === 1) {
          ...
          pdfContent.push({ text: vm.$t('exportTrophyGazette.cip_facility'), style: 'td', alignment: 'left', bold: true })
          pdfContent.push({ text: i18n.locale === 'bn' ? vm.form.cip_facilities_bn.replaceAll('<br />', '') : vm.form.cip_facilities_en.replaceAll('<br />', ''), style: 'td', alignment: 'left' })

          pdfContent.push({ text: vm.$t('exportTrophyGazette.cip_timeline'), style: 'td', alignment: 'left', bold: true })
          pdfContent.push({ text: i18n.locale === 'bn' ? vm.form.timeline_bn.replaceAll('<br />', '') : vm.form.timeline_en.replaceAll('<br />', ''), style: 'td', alignment: 'left' })
        }

        if (vm.form.type === 2) {
          let dataCount = 1
          for (const prop in vm.etApps) {
            pdfContent.push({ text: '(' + vm.$n(dataCount++, { useGrouping: false }) + ') ' + vm.$t('exportTrophyGazette.product_sector') + ': ' + vm.getProductName(parseInt(prop)), style: 'td', alignment: 'left' })
            const allRowsHead = [
              [
                { text: vm.$t('globalTrans.sl_no'), style: 'th', alignment: 'left' },
