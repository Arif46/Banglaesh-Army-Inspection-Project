  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exportTrophyConfig.assing_income') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophyConfig.assing_income') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { assignIncomeListApi, assignIncomeToggleApi } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('exportTrophyConfig.assing_income') + ' ' + this.$t('globalTrans.entry') : this.$t('exportTrophyConfig.assing_income') + ' ' + this.$t('globalTrans.modify')
    },
    detailsTitle () {
      return this.$t('exportTrophyConfig.awardType') + ' ' + this.$t('globalTrans.details')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, assignIncomeListApi, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, exportTrophyCIPServiceBaseUrl, assignIncomeToggleApi, item)
    },
    details (item) {
