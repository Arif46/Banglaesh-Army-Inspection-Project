  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exportTrophyConfig.report_heading') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exportTrophyConfig.report_heading') }} {{ $t('globalTrans.list') }}</h4>
      </template>
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(left_logo)="data">
                    <div class="text-center">
                      <img :src="exportTrophyCIPServiceBaseUrl+'storage' + data.item.left_logo" width="50"/>
                    </div>
                  </template>
                  <template v-slot:cell(right_logo)="data">
                    <div class="text-center">
                      <img :src="exportTrophyCIPServiceBaseUrl+'storage' + data.item.right_logo" width="50"/>
                    </div>
                  </template>
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.globalTrans.address, label_bn: i18n.messages.bn.globalTrans.address, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '40%' } },
  { label_en: i18n.messages.en.globalTrans.left_logo, label_bn: i18n.messages.bn.globalTrans.left_logo, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '40%' } },
  { label_en: i18n.messages.en.globalTrans.right_logo, label_bn: i18n.messages.bn.globalTrans.right_logo, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '40%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('exportTrophyConfig.report_heading') + ' ' + this.$t('globalTrans.entry') : this.$t('exportTrophyConfig.report_heading') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(exportTrophyCIPServiceBaseUrl, reportHeadingListApi, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, exportTrophyCIPServiceBaseUrl, reportHeadingToggleApi, item)
    }
  }
}
</script>
