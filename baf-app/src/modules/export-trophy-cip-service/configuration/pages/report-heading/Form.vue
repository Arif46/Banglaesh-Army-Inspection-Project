        ...
        <b-row>
            <b-col sm="6">
                <ValidationProvider name="Heading (En)" vid="heading_en" rules="required">
                    ...
                    >
                    <template v-slot:label>
                        {{$t('exportTrophyConfig.heading_en')}} <span class="text-danger">*</span>
                    </template>
                    <b-form-input
                        plain
            ...
            <b-col sm="6">
                <ValidationProvider name="Heading (Bn)" vid="heading_bn" rules="required">
                    ...
                    >
                        <template v-slot:label>
                            {{$t('exportTrophyConfig.heading_bn')}} <span class="text-danger">*</span>
                        </template>
                        <b-form-input
                            plain
            ...
            <b-col sm="6">
                <ValidationProvider name="Address (En)" vid="address_en"  rules="required">
                    ...
                    >
                    <template v-slot:label>
                        {{$t('exportTrophyConfig.address_en')}} <span class="text-danger">*</span>
                    </template>
                    <b-form-textarea
                        id="address_en"
            ...
            <b-col sm="6">
                <ValidationProvider name="Address (Bn)" vid="address_bn" rules="required">
                    ...
                    >
                    <template v-slot:label>
                        {{$t('exportTrophyConfig.address_bn')}} <span class="text-danger">*</span>
                    </template>
                    <b-form-textarea
                        id="address_bn"
...
export default {
  ...
  methods: {
    ...
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(exportTrophyCIPServiceBaseUrl, `${reportHeadingUpdateApi}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(exportTrophyCIPServiceBaseUrl, reportHeadingStoreApi, this.form)
      }
      ...
      if (result.warning) {
        this.$toast.error({
          title: 'Warning',
          message: this.$t('exportTrophyConfig.alradyMsg'),
          color: '#e77171'
        })
        return
      }
