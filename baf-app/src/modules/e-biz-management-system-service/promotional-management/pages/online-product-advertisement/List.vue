...
import { opdPromotionalList, opdPromotionalApprove } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    viewTitle () {
      return (this.viewitemId === 0) ? this.$t('eBizPromotional.online_product') + ' ' + this.$t('globalTrans.view') : this.$t('eBizPromotional.online_product') + ' ' + this.$t('globalTrans.modify')
    },
    formTitle () {
      return this.$t('eBizPromotional.online_product') + ' ' + this.$t('globalTrans.approve')
    },
    councilInfoList () {
  ...
  methods: {
    ...
    async approve (item, type) {
        ...
        let result = null
        result = await RestApi.postData(eBizServiceBaseUrl, opdPromotionalApprove, item)
        if (result.success) {
    ...
    async loadData () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(eBizServiceBaseUrl, opdPromotionalList, params)
      if (result.success) {
