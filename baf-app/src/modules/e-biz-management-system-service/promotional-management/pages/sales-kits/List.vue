...
import { skPromotionalList, skPromotionalApprove } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'associationPnl.advertisement_title', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '13%' } },
  ...
  computed: {
    ...
    viewTitle () {
      return (this.viewitemId === 0) ? this.$t('eBizPromotional.sales_kits') + ' ' + this.$t('globalTrans.view') : this.$t('eBizPromotional.sales_kits') + ' ' + this.$t('globalTrans.modify')
    },
    formTitle () {
      return this.$t('eBizPromotional.sales_kits') + ' ' + this.$t('globalTrans.approve')
    },
    councilInfoList () {
  ...
  methods: {
    ...
    async approve (item, type) {
        ...
        let result = null
        result = await RestApi.postData(eBizServiceBaseUrl, skPromotionalApprove, item)
        if (result.success) {
    ...
    async loadData () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(eBizServiceBaseUrl, skPromotionalList, params)
      if (result.success) {
