...
import { prPromotionalList, prPromotionalApprove } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    viewTitle () {
      return (this.viewitemId === 0) ? this.$t('eBizPromotional.public_relationship_management') + ' ' + this.$t('globalTrans.view') : this.$t('eBizPromotional.public_relationship_management') + ' ' + this.$t('globalTrans.modify')
    },
    formTitle () {
      return this.$t('eBizPromotional.public_relationship_management') + ' ' + this.$t('globalTrans.approve')
    },
    councilInfoList () {
  ...
  methods: {
    ...
    async approve (item, type) {
        ...
        let result = null
        result = await RestApi.postData(eBizServiceBaseUrl, prPromotionalApprove, item)
        if (result.success) {
    ...
    async loadData () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(eBizServiceBaseUrl, prPromotionalList, params)
      if (result.success) {
