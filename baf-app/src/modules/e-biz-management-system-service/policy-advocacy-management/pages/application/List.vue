              ...
              <slot v-if="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <!-- <b-button v-if="data.item.status < 2" v-b-modal.modal-form variant="iq-bg-success mr-1" size="sm" @click="edit(data.item)" class="action-btn edit" :title="$t('globalTrans.edit')"><i class="ri-arrow-right-line"></i></b-button> -->
                    ...
                    <template v-if="data.item.status < 3">
                      <b-button variant="iq-bg-success mr-1" size="sm" @click="confirmForwardOrReject(data.item.id,1)" class="action-btn edit btn-success" :title="$t('globalTrans.forward')"><i class="ri-arrow-right-line"></i>
                      </b-button>
                      <b-button variant="iq-bg-danger mr-1" size="sm" @click="confirmForwardOrReject(data.item.id,2)" class="action-btn edit btn-danger" :title="$t('globalTrans.reject')"><i class="ri-close-line"></i>
                      </b-button>
                    </template>
                  </template>
                </b-table>
...
import { applicationList, applicationForward, applicationReject } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, applicationList, params)
      if (result.success) {
    ...
    async forward (appId) {
      ...
      const result = await RestApi.postData(eBizServiceBaseUrl, `${applicationForward}/${parseInt(appId)}`)
      this.loading = false
  ...
  async reject (appId) {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true })
      const loadingState = { loading: false, listReload: false }
      const result = await RestApi.postData(eBizServiceBaseUrl, `${applicationReject}/${parseInt(appId)}`)
      this.loading = false
      loadingState.listReload = true
      this.$store.dispatch('mutateCommonProperties', loadingState)
      if (result.success) {
