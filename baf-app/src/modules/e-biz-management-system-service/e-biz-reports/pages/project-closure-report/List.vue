  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{$t('eBizReports.project_closure_report')}}</h4>
      </template>
    ...
    <div>
        ...
        <export-excel
                ...
                name="Project_closure_report.xls">
        ...
        <div class="table-wrapper table-responsive" >
          ...
          <tr>
            <th colspan="15" style="text-align: center;">{{ $t('eBizReports.project_closure_report') }}</th>
          </tr>
          <tr>
...
import { ProjectClosureReport } from '../../api/routes'
...
export default {
  ...
  computed: {
    headerValue: function () {
      ...
      if (this.$i18n.locale === 'en') {
        ...
        headerVal[3] = ''
        headerVal[4] = ''
        headerVal[5] = this.$t('eBizReports.project_closure_report')
        headerVal[6] = ''
      } else {
        headerVal[0] = this.$t('tcb_report.government_text')
        headerVal[1] = this.$t('eBizReports.bpc_text')
  ...
  excelData: function () {
      ...
      serial++
          ...
          return {
              ...
              'Proposal No': '000' + item.proposal_id.toString(),
              'Program No ': this.$n(item.program_id),
              'Program Name ': item.programe_name_en,
              'Start Date ': this.dateFormat(item.start_date),
              'End Date ': this.dateFormat(item.end_date)
          }
          } else {
          return {
              'ক্রমিক নং ': this.$n(serial),
              'অর্থবছর ': this.getFiscalYear(item.fiscal_year_id),
              'প্রপোজাল নং': '000' + item.proposal_id.toString(),
              'প্রোগ্রাম আইডি ': this.$n(item.program_id),
              'প্রোগ্রামের নাম ': item.programe_name_bn,
              'শুরুর তারিখ ': this.dateFormat(item.start_date),
              'শেষের তারিখ ': this.dateFormat(item.end_date)
          }
          }
      })
  ...
  methods: {
    async searchData () {
      ...
      result = await RestApi.getData(eBizServiceBaseUrl, ProjectClosureReport, params)
      if (result.success) {
