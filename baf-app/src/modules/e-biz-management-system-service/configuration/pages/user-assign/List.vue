  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('eBizConfig.user_assign') }} {{$t('globalTrans.search')}}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('eBizConfig.user_assign') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal :no-close-on-backdrop="true" id="modal-form" size="lg" :title="formTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
    ...
    <b-modal id="modal-details" size="lg" :title="viewTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
     <Details :items="items" :users="users" :key="viewitemId"/>
    </b-modal>
  </div>
</template>
<script>
...
import { eBizUserAssignList, eBizUserAssignToggle } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'globalTrans.user', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '30%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('eBizConfig.user_assign') + ' ' + this.$t('globalTrans.entry') : this.$t('eBizConfig.user_assign') + ' ' + this.$t('globalTrans.modify')
    },
    viewTitle () {
      return (this.editItemId === 0) ? this.$t('eBizConfig.user_assign') + ' ' + this.$t('globalTrans.details') : this.$t('eBizConfig.user_assign') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
      getUserName (userId) {
      ...
      remove (item) {
        this.changeStatus(3, eBizServiceBaseUrl, eBizUserAssignToggle, item)
      },
    searchData () {
      this.loadData()
    },
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, eBizUserAssignList, params)
      if (result.success) {
