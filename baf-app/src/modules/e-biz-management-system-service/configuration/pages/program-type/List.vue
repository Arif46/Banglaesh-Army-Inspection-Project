  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('eBizConfig.program_type') }} {{ $t('globalTrans.search')}}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('eBizConfig.program_type') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(program_name)="data">
                    {{ $i18n.locale === 'bn' ? data.item.program_name_bn : data.item.program_name_en }}
                  </template>
...
import { programTypeList, programTypeToggle } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'eBizConfig.program_name', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '15%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('eBizConfig.program_type') + ' ' + this.$t('globalTrans.entry') : this.$t('eBizConfig.program_type') + ' ' + this.$t('globalTrans.modify')
    },
    viewTitle () {
      return (this.viewitemId === 0) ? this.$t('eBizConfig.program_type') + ' ' + this.$t('globalTrans.view') : this.$t('eBizConfig.program_type') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(3, eBizServiceBaseUrl, programTypeToggle, item)
      }
    },
    details (item) {
        this.items = item
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, programTypeList, params)
      if (result.success) {
