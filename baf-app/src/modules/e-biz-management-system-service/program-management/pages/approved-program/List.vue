                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                 ...
                 <template v-slot:cell(program_name)="data">
                  ...
                  <template v-slot:cell(status)="data">
                    ...
                    <span class="badge badge-primary" v-if="data.item.status === 4" >{{$t('globalTrans.approve')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button variant="iq-bg-success mr-1" size="sm" class="action-btn btn-success" :title="$t('globalTrans.approve')" @click="approved(data.item.id)"> <i class="ri-check-line"></i>
                    </b-button>
                  </template>
                </b-table>
              </slot>
...
import { pendingProgramList, programApproved } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    approved (id) {
          window.vm.$swal({
              title: window.vm.$t('eBizProgram.approve_msg'),
      ...
      saveApproved (id) {
          ...
          RestApi.putData(eBizServiceBaseUrl, `${programApproved}/${id}`).then(response => {
              if (response.success) {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, pendingProgramList, params)
      if (result.success) {
