<template>
  <div class="inner-section">
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                 ...
                 <template v-slot:cell(program_name)="data">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-primary" v-if="data.item.status > 4" >{{ $i18n.locale === 'en' ? 'Agreement' : 'চুক্তি'}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    <b-button  v-b-modal.details variant="action-btn status mr-1" size="sm" @click="details(data.item)" class="action-btn status" :title="$t('globalTrans.details')"><i class="ri-eye-fill"></i>
                     ...
                     <b-button v-if="data.item.investigators?.length > 0 && data.item.evaluations?.length <= 0"  v-b-modal.evaluation-form @click="evaluation(data.item)" class="btn btn-sm btn-success"> {{ $t('eBizProgram.evaluation') }}
                    </b-button>
                    <b-button v-if="data.item.evaluations?.length > 0" v-b-modal.evaluation-view variant="action-btn status mr-1" size="sm" @click="viewEvaluation(data.item)" class="btn btn-md btn-info"> {{ $t('eBizProgram.evaluation') + ' ' + $t('globalTrans.view') }}
                    </b-button>
                    <!-- <b-button v-if="data.item.investigators?.length > 0" class="btn btn-sm btn-warning"> {{ $t('eBizProgram.already_assign') }}
                    </b-button> -->
                  </template>
                </b-table>
              </slot>
   ...
   <b-modal id="evaluation-view" size="lg" :title="evaluationViewTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
    <EvaluationView :items="items" :key="viewitemId"/>
   </b-modal>
    <b-modal id="evaluation-form" size="lg" :title="evaluationTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Form :items="items" :key="viewitemId"/>
    </b-modal>
  </div>
</template>
<script>
import Details from './Details.vue'
import EvaluationView from './ViewEvaluation.vue'
import Form from './Form.vue'
import RestApi, { eBizServiceBaseUrl } from '@/config/api_config'
import { evaluationList } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    evaluationViewTitle () {
      return this.$t('eBizProgram.evaluation') + ' ' + this.$t('globalTrans.view')
    },
    evaluationTitle () {
      return this.$t('eBizProgram.monitoring_and_evaluatiion')
    },
    councilInfoList () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, evaluationList, params)
      if (result.success) {
