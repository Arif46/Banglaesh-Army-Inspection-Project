      ...
      <ValidationObserver ref="mainForm" v-slot="{ handleSubmit, reset }">
        ...
        <b-row>
          <b-col xs="12" sm="12" md="6">
            <ValidationProvider name="Remark" vid="send_remark" v-slot="{ errors }" rules="required">
              <b-form-group
                label-for="send_remark">
        ...
        <b-row class="text-right">
            <b-col>
                ...
                <b-button variant="danger" class="btn-sm" @click="$bvModal.hide('send')">{{ $t('globalTrans.cancel') }}</b-button>
            </b-col>
        </b-row>
        </b-overlay>
    </b-row>
...
import { programSend } from '../../api/routes'
export default {
  name: 'Form',
  props: ['items'],
  data () {
    return {
      valid: null,
      saveBtnName: this.items.id ? this.$t('globalTrans.send') : this.$t('globalTrans.save'),
  ...
  methods: {
    ...
    async saveData () {
      ...
      if (this.items.id) {
        result = await RestApi.putData(eBizServiceBaseUrl, `${programSend}/${this.items.id}`, this.formData)
      }
