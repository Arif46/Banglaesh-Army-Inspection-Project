<template>
  <div class="inner-section">
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                 ...
                 <template v-slot:cell(program_name)="data">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button  v-b-modal.make-agreement @click="agreement(data.item)" class="btn btn-sm btn-success"> {{ $t('eBizProgram.agreement') }}
                    </b-button>
                  </template>
                </b-table>
              </slot>
   ...
   <b-modal id="details" size="lg" :title="viewTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
    ...
    <b-modal id="make-agreement" size="lg" :title="agreementTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <agreement-form :items="items" :key="viewitemId"/>
    </b-modal>
  </div>
</template>
<script>
...
import { approvedProgramList } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    agreementTitle () {
      return this.$t('eBizProgram.agreement')
    },
    councilInfoList () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(eBizServiceBaseUrl, approvedProgramList, params)
      if (result.success) {
