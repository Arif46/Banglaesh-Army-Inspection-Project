<template>
  <div class="inner-section">
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                 ...
                 <template v-slot:cell(program_name)="data">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button v-if="data.item.investigators?.length < 1"  v-b-modal.investigator-form @click="investigator(data.item)" class="btn btn-sm btn-success"> {{ $t('eBizProgram.assign_investigator') }}
                    </b-button>
                     <b-button v-if="data.item.investigators?.length > 0"  v-b-modal.investigator-view @click="investigator(data.item)" class="btn btn-sm btn-success"> {{ $t('eBizProgram.view_investigator') }}
                    </b-button>
   ...
   <b-modal id="investigator-view" size="lg" :title="viewTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
    <InvestigatorView :items="items" :key="viewitemId"/>
   </b-modal>
    <b-modal id="investigator-form" size="lg" :title="investigatorTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import InvestigatorView from './ViewInvestigator.vue'
...
export default {
  ...
  computed: {
    ...
    investigatorTitle () {
      return this.$t('eBizProgram.assign_investigator')
    },
    councilInfoList () {
