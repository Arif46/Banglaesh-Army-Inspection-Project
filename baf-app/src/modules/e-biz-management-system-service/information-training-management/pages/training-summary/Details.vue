                ...
                <b-row>
                    <b-col md="12">
                        <table style="line-height: 25px" class="table table-sm table-borderless">
                            ...
                            <!-- <pre>{{ items }}</pre> -->
                            <tr>
                                <th style="width: 20%">{{$t('eBizConfig.program_name')}}</th>
                                <td style="width: 35%" > : {{currentLocale == 'bn' ? items.information.program_name_bn : items.information.program_name}}</td>
                            </tr>
                            <tr>
                                <th style="width: 20%">{{$t('informationTraining.training_description')}}</th>
                                <td style="width: 35%" > : {{currentLocale == 'bn' ? items.training_description_bn : items.training_description}}</td>
                            </tr>
                            <tr>
                                <th style="width: 20%">{{ $t('informationTraining.training_documents')}} </th>
                                <td style="width: 35%">:
                                       <span v-if="items.training_documents !== null"><a target="_blank" class="btn btn-primary mr-2" :href="eBizServiceBaseUrl+'download-attachment?file='+items.training_documents" title="Training Document"><i class="ri-file-download-line"></i>&nbsp;</a></span>
