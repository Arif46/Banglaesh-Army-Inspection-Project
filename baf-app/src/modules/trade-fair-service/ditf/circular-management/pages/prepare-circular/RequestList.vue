  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('ditfTradeFairManages.ditf_req_circular') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('ditfTradeFairManages.ditf_req_circular') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { prepareCircularForwardList } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'globalTrans.status', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 54, thStyle: { width: '10%' } },
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, prepareCircularForwardList, params)
      if (result.success) {
