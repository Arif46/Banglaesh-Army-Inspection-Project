...
import { prepareCircularForwardCommentsApi } from '../../api/routes'
export default {
  name: 'Details',
  props: ['item'],
  data () {
  ...
  methods: {
    getForwardComments (circularId) {
      ...
      RestApi.getData(internationalTradeFairServiceBaseUrl, `${prepareCircularForwardCommentsApi}/${circularId}`).then(response => {
        if (response.success) {
