  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('ditfTradeFairManages.ditf_circular') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('ditfTradeFairManages.ditf_circular') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { prepareCircularList, prepareCircularPublish } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('ditfTradeFairManages.ditf_circular') + ' ' + this.$t('globalTrans.entry') : this.$t('ditfTradeFairManages.ditf_circular') + ' ' + this.$t('globalTrans.modify')
    },
    yearList () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, prepareCircularList, params)
      if (result.success) {
    ...
    publishStatus (item) {
      window.vm.$swal({
        ...
        if (result.isConfirmed) {
          ...
          result = RestApi.postData(internationalTradeFairServiceBaseUrl, prepareCircularPublish, item)
