  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('ditfConfig.business_type') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('ditfConfig.business_type') }}</h4>
      </template>
...
<script>
  ...
  import { businessTypeList, businessTypeToggle } from '../../api/routes'
  ...
  export default {
    ...
    computed: {
      labelList: function () {
        ...
        const defaultColumn = [
          ...
          { label_en: i18n.messages.en.ditfConfig.business_type, label_bn: i18n.messages.bn.ditfConfig.business_type, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '25%' } },
          { label_en: i18n.messages.en.globalTrans.status, label_bn: i18n.messages.bn.globalTrans.status, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '15%' } },
      ...
      formTitle () {
        return (this.editItemId === 0) ? this.$t('ditfConfig.business_type') + ' ' + this.$t('globalTrans.entry') : this.$t('ditfConfig.business_type') + ' ' + this.$t('globalTrans.modify')
      },
      pageOptions () {
    ...
    methods: {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, businessTypeList, params)
        if (result.success) {
      ...
      remove (item) {
        this.changeStatus(9, internationalTradeFairServiceBaseUrl, businessTypeToggle, item)
      }
    }
  }
</script>
