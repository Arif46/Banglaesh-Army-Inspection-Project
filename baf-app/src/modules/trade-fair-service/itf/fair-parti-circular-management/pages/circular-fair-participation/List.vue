  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairPartiCircularManagement.fair_parti_circular') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairPartiCircularManagement.fair_parti_circular') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { itfFairPartiCircularList, fairPartiCircularPublish } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, itfFairPartiCircularList, params)
      if (result.success) {
    ...
    publishStatus (item) {
      window.vm.$swal({
        ...
        if (result.isConfirmed) {
          this.changeStatus(internationalTradeFairServiceBaseUrl, fairPartiCircularPublish, item)
        }
      })
    },
    pdfExport () {
