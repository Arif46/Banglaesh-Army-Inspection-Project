  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairPartiCircularManagement.fair_parti_circular_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairPartiCircularManagement.fair_parti_circular_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { itfFairPartiCircularForwardList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, itfFairPartiCircularForwardList, params)
      if (result.success) {
