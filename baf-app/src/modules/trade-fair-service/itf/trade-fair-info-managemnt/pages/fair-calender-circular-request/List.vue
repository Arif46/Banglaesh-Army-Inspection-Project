  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairInfoManagement.fair_calender_circular_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairInfoManagement.fair_calender_circular_request') }}  {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-secondary" v-if="data.item.status == 2">{{$t('globalTrans.pending')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    <b-button v-b-modal.modal-form-details variant="iq-bg-success" size="sm" @click="edit(data.item)" class="action-btn status" v-b-tooltip.hover :title="$t('globalTrans.view')"><i class="ri-eye-fill"></i></b-button>
                    <b-button :title="$t('globalTrans.forward')" v-b-modal.modal-5 variant="iq-bg-info" class="action-btn btn-info" size="sm" @click="edit(data.item)"><i class="ri-share-forward-line"></i></b-button>
                    <b-button :title="$t('globalTrans.approve')" v-b-modal.modal-6 variant="iq-bg-info" class="action-btn btn-success" size="sm" @click="edit(data.item)"><i class="ri-check-line"></i></b-button>
                    <b-button :title="$t('globalTrans.return')" v-b-modal.modal-7 variant=" iq-bg-success border-info" class="action-btn btn-warning" size="sm" @click="returnData(data.item)"><i class="ri-arrow-go-back-fill"></i></b-button>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-form-details" size="lg" :title="$t('globalTrans.details')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { fairCalenderCircularForwardList } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.globalTrans.status, label_bn: i18n.messages.bn.globalTrans.status, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '12%' } },
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, fairCalenderCircularForwardList, params)
      if (result.success) {
