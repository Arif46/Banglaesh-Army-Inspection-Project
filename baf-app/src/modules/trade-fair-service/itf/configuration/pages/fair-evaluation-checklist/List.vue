  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.evaluation_checklist') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.evaluation_checklist') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal id="modal-form" size="lg" :title="formTitle" hide-footer ok-only ok-variant="danger">
      <Form :id="detailsItemId" :key="detailsItemId" />
    </b-modal>
    <b-modal id="modal-4" size="lg" :title="$t('tradeFairConfig.checklist_criteria_details')" hide-footer ok-only ok-variant="danger">
...
import { evaluationChecklistList, evaluationChecklistToggle } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.detailsItemId === 0) ? this.$t('tradeFairConfig.evaluation_checklist') + ' ' + this.$t('globalTrans.entry') : this.$t('tradeFairConfig.evaluation_checklist') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, evaluationChecklistList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, internationalTradeFairServiceBaseUrl, evaluationChecklistToggle, item)
    }
  }
}
</script>
