...
export default {
  ...
  created () {
    if (this.id) {
      ...
      this.form = tmp
      this.form.pre_left_logo = tmp.left_logo
      this.form.pre_right_logo = tmp.right_logo
      this.form.left_logo = ''
  ...
  methods: {
    ...
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(internationalTradeFairServiceBaseUrl, `${reportHeadUpdate}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(internationalTradeFairServiceBaseUrl, reportHeadStore, this.form)
      }
      ...
      if (result.success) {
        this.$store.dispatch('internationalTradeFairService/mutateCommonObj', { hasDropdownLoaded: false })
        this.$toast.success({
