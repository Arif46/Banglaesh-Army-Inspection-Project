  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.subsidy_information') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.subsidy_info_list') }}</h4>
      </template>
    ...
    <b-modal id="modal-4" size="xl" :title="$t('tradeFairConfig.subsidy_info_details')" hide-footer ok-only ok-variant="danger">
...
import { subsidyInformationList, subsidyInformationToggle } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.detailsItemId === 0) ? this.$t('tradeFairConfig.subsidy_information') + ' ' + this.$t('globalTrans.entry') : this.$t('tradeFairConfig.subsidy_information') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    getFairNameList () {
      return this.$store.state.TradeFairService.commonObj.fairNameList.filter(item => item.fiscal_year_id === this.search.fiscal_year).map(el => {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, subsidyInformationList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, internationalTradeFairServiceBaseUrl, subsidyInformationToggle, item)
    }
  }
}
</script>
