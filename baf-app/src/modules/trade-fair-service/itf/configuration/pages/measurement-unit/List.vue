  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.measurement_unit') }} {{ $t('globalTrans.search') }}</h4>
      </template>
          ...
          <b-col sm="4">
            ...
            >
              <template v-slot:label>
                {{ $t('tradeFairConfig.measurement_unit') }}
              </template>
              <b-form-input
                  id="unit_name"
                  type="text"
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.measurement_unit_list') }}</h4>
      </template>
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.tradeFairConfig.measurement_unit_name, label_bn: i18n.messages.bn.tradeFairConfig.measurement_unit_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '25%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('tradeFairConfig.measurement_unit') + ' ' + this.$t('globalTrans.entry') : this.$t('tradeFairConfig.measurement_unit') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, measurementUnitList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, internationalTradeFairServiceBaseUrl, measurementUnitToggle, item)
    }
  }
}
</script>
