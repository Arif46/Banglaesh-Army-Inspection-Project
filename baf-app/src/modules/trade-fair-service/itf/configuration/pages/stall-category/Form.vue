                ...
                <b-form @submit.prevent="handleSubmit(addItem)" @reset.prevent="reset" autocomplete="off">
                  <b-row class="p-4">
                    <b-col style="background-color: var(--gray-ddd);" lg="6" class="offset-lg-3 p-2">
                      <template>
                        <h6 style="" class="text-sm-center">{{ $t('tradeFairConfig.stall_cat_name') }} {{ $t('globalTrans.entry')}}</h6>
                      </template>
                    </b-col>
                  </b-row>
                  <b-row>
                    <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                      <ValidationProvider name="Stall Cat Name (En)" vid="stall_cat_name_en" rules="required" v-slot="{ errors }">
                        <b-form-group
                          label-for="stall_cat_name">
                          <template v-slot:label>
                            {{ $t('tradeFairConfig.stall_cat_name') }} {{ $t('globalTrans.en') }} <span class="text-danger">*</span>
                          </template>
                          <b-form-input
                            id="stall_cat_name_en"
                            v-model="detailsFormData.stall_cat_name_en"
                            :state="errors[0] ? false : (valid ? true : null)"
                          ></b-form-input>
                          <div class="invalid-feedback">
                            {{ errors[0] }}
                    ...
                    <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                      <ValidationProvider name="Stall Cat Name (Bn)" vid="stall_cat_name_bn" rules="required" v-slot="{ errors }">
                        <b-form-group
                          label-for="stall_cat_name_bn">
                          <template v-slot:label>
                            {{ $t('tradeFairConfig.stall_cat_name') }} {{ $t('globalTrans.bn') }} <span class="text-danger">*</span>
                          </template>
                          <b-form-input
                            id="stall_cat_name_bn"
                            v-model="detailsFormData.stall_cat_name_bn"
                            :state="errors[0] ? false : (valid ? true : null)"
                          ></b-form-input>
                          <div class="invalid-feedback">
                            {{ errors[0] }}
            ...
            <b-table-simple bordered>
              ...
              <b-tbody>
                  <b-tr v-for="(item, index) in data.details" :key="index">
                      ...
                      <b-td>
                        {{ currentLocale == 'bn' ? item.stall_cat_name_bn : item.stall_cat_name_en }}
                      </b-td>
                      <b-td>
...
export default {
  ...
  methods: {
    ...
    async addItem () {
      ...
      const stallCategoryNameEn = this.detailsFormData.stall_cat_name_en
      const newData = result.find(item => item.stall_cat_name_en === stallCategoryNameEn)
      if (typeof newData === 'undefined') {
          if (this.editIndex !== '') {
              // this.data.details[parseInt(this.editIndex)] = this.detailsFormData
              this.data.details[parseInt(this.editIndex)].stall_cat_name_en = this.detailsFormData.stall_cat_name_en
              this.data.details[parseInt(this.editIndex)].stall_cat_name_bn = this.detailsFormData.stall_cat_name_bn
          } else {
    ...
    async getStallCategoryInfo () {
      ...
      const detailsData = tmpData.details.map(item => {
        return {
          stall_cat_name_en: item.stall_cat_name_en, stall_cat_name_bn: item.stall_cat_name_bn
        }
      })
