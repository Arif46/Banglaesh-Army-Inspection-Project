  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.stall_information') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.stall_info_list') }}</h4>
      </template>
...
import { stallInformationList, stallInformationToggle } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.detailsItemId === 0) ? this.$t('tradeFairConfig.stall_information') + ' ' + this.$t('globalTrans.entry') : this.$t('tradeFairConfig.stall_information') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    getFairNameList () {
      return this.$store.state.TradeFairService.commonObj.fairNameList.filter(item => item.fiscal_year_id === this.search.fiscal_year)
    },
    resetId () {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, stallInformationList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, internationalTradeFairServiceBaseUrl, stallInformationToggle, item)
    }
  }
}
</script>
