  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.fair_sector') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('tradeFairConfig.fair_sector_list') }}</h4>
      </template>
...
import { fairSectorList, fairSectorToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.tradeFairConfig.fair_sector, label_bn: i18n.messages.bn.tradeFairConfig.fair_sector, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '25%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('tradeFairConfig.fair_sector') + ' ' + this.$t('globalTrans.entry') : this.$t('tradeFairConfig.fair_sector') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, fairSectorList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(9, internationalTradeFairServiceBaseUrl, fairSectorToggle, item)
    }
  }
}
</script>
