...
import { itfFairExhibitCircularForwardCommentsApi } from '../../api/routes'
export default {
  name: 'Details',
  props: ['item'],
  data () {
  ...
  methods: {
    getForwardComments (circularId) {
      ...
      RestApi.getData(internationalTradeFairServiceBaseUrl, `${itfFairExhibitCircularForwardCommentsApi}/${circularId}`).then(response => {
        if (response.success) {
