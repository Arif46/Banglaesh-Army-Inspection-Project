<template>
  <ValidationObserver ref="form" v-slot="{ handleSubmit, reset }">
    ...
    <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off">
      <b-row>
        <b-col sm="4">
            <ValidationProvider name="Office" vid="office_id" rules="required|min_value:1" v-slot="{ errors }">
              <b-form-group
                class="row"
                label-for="office_id"
              >
        ...
        <b-col sm="6">
          <ValidationProvider name="Note (En)" vid="note_en" rules="max:255">
            <b-form-group
              label-for="note_en"
              slot-scope="{ errors }"
            >
        ...
        <b-col sm="6">
          <ValidationProvider name="Note (Bn)" vid="note_bn" rules="max:255">
            <b-form-group
              label-for="note_bn"
              slot-scope="{ errors }"
            >
...
import { itfFairExhibitCircularForward, userListByDesignationApi } from '../../api/routes'
export default {
  name: 'Form',
  props: ['item'],
  data () {
  ...
  created () {
    if (this.item) {
      this.form.circular_id = this.item.id
  ...
  methods: {
    ...
    async saveUpdate () {
      ...
      result = await RestApi.postData(internationalTradeFairServiceBaseUrl, itfFairExhibitCircularForward, this.form)
