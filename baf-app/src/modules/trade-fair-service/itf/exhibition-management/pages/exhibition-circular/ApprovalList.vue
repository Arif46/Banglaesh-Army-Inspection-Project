  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exhibitCircularManagement.app_cir_of_exit') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exhibitCircularManagement.app_cir_of_exit') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { itfFairExhibitCircularApprovalList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, itfFairExhibitCircularApprovalList, params)
      if (result.success) {
