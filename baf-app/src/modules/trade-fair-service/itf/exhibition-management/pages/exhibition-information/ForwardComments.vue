...
import { exhibitionForwardCommentApi } from '../../api/routes'
export default {
  name: 'Details',
  props: ['item'],
  data () {
  ...
  methods: {
    getForwardComments (circularId) {
      ...
      RestApi.getData(internationalTradeFairServiceBaseUrl, `${exhibitionForwardCommentApi}/${circularId}`).then(response => {
        if (response.success) {
