  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exhibitCircularManagement.fair_exhibition_info') }} {{ $t('globalTrans.search') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    ...
                    <span class="badge badge-info" v-else-if="data.item.status == 5">{{$t('exhibitCircularManagement.resubmit')}}</span>
                  </template>
                  ...
                  <template #custom-foot="">
                    <b-tr>
                      <b-th colspan="10" class="text-right">
                        <button v-b-modal.modal-forward class="btn btn-primary btn-sm" :title="$t('globalTrans.forward')" @click="forward(selectedItem)" :disabled="selectedItem.length > 0 ? false : true">
...
import { fairExhibitInfoList } from '../../api/routes'
import ModalBaseMasterList from '@/mixins/list'
export default {
  mixins: [ModalBaseMasterList],
  components: {
    Details, Forward, ForwardComments
  },
  data () {
    return {
      sortBy: '',
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, fairExhibitInfoList, params)
      if (result.success) {
