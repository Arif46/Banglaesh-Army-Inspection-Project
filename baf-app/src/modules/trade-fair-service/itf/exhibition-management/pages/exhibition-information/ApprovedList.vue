  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('exhibitCircularManagement.approved_fair_exhibition') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('exhibitCircularManagement.approved_fair_exhibition') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal id="modal-details" size="lg" :title="$t('globalTrans.details')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <template #modal-title class="flex-grow-1">
        {{ $t('exhibitCircularManagement.approved_fair_exhibition_info') + ' ' + $t('globalTrans.details') }}
...
import { fairExhibitInfoApprovalList } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'externalTradeFair.exhibition_id', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '20%' } },
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, fairExhibitInfoApprovalList, params)
      if (result.success) {
