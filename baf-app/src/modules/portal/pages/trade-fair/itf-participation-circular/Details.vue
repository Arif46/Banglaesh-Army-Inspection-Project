              ...
              <div>
                <b-table-simple bordered hover small>
                  ...
                  <b-tbody v-for="(item, index) in stall_info" :key="index">
                    <tr v-for="(detail, indx) in item.details" :key="indx">
                      <b-td class="text-center align-middle" v-if="indx == 0" :rowspan="item.details.length" style="width: 8%">{{ $n(index + 1) }}</b-td>
                      <b-td class="text-center align-middle" v-if="indx == 0" :rowspan="item.details.length" style="width: 15%">{{ getStallCategoryName(item.stall_cat_id) }}</b-td>
                      <b-td class="text-center" style="width: 10%">{{ ($n(detail.stall_size)) + ' ' + getMeasurementUnitList(detail.measurement_id) }}</b-td>
                      <b-td class="text-center" style="width: 15%">{{ getStallType(detail.stall_type) }}</b-td>
                      <b-td class="text-center" style="width: 10%">{{ $n(detail.booth_rent, { minimumFractionDigits: 2 }) }}</b-td>
                      <b-td class="text-center" style="width: 27%">{{ $i18n.locale === 'en' ? detail.description_en : detail.description_bn }}</b-td>
                    </tr>
                  </b-tbody>
                </b-table-simple>
                <b-row class="mt-3 mb-3">
...
export default {
    ...
    methods: {
      getSubsidyCategory (id) {
        const dataSubsidyCat = this.dropdown.subsidyCategoryList.find(item => item.value === parseInt(id))
      ...
      getMeasurementUnitList (id) {
        const dataMeasurementUnit = this.dropdown.measurementUnitList.find(item => item.value === parseInt(id))
      ...
      getFairSector (id) {
        const dataFairSector = this.dropdown.fairSectorList.find(item => item.value === parseInt(id))
      ...
      getStallCategoryList (id) {
        RestApi.getData(internationalTradeFairServiceBaseUrl, '/itf-configuration/stall-category', { fair_name_id: id }).then(response => {
          if (response.success) {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, '/portal/itf-participation-circular-details', params)
        this.loading = false
        if (result.success) {
          this.detailsData = result.data
          this.mappingStallInfo(this.detailsData.stall_info.details)
          const country = this.$store.state.Portal.commonObj.countryList.find(obj => obj.value === parseInt(this.detailsData.calendar_info.country_id))
          this.country_en = country ? country.text_en : ''
          this.country_bn = country ? country.text_bn : ''
      ...
      pdfExport () {
        ...
        ExportPdf.exportPdfDetails(internationalTradeFairServiceBaseUrl, '/itf-report-head/report-head/detail', 2, reportTitle, this)
      }
    }
}
</script>
