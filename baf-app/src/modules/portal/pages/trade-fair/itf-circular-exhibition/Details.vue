    ...
    methods: {
      ...
      async loadData () {
        ...
        if (result.success) {
          ...
          const fiscalYear = this.$store.state.CommonService.commonObj.fiscalYearList.find(obj => obj.value === parseInt(result.data.fiscal_year_id))
          const fairName = this.$store.state.ExternalUserService.tradeFair.commonObj.fairNameList.find(obj => obj.value === parseInt(result.data.parti_circular.fair_name_id))
