    ...
    <div class="service-details-wrapper">
                    ...
                    <b-col md="12" class="table-responsive">
                      <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                        ...
                        <template v-slot:cell(action)="data">
                          ...
                          <b-button :title="$t('globalTrans.apply')" @click="setServiceUrl(data.item.id)" class="btn btn-success ml-1" >{{ $t('globalTrans.apply') }}</b-button>
                        </template>
                      </b-table>
        ...
        <b-modal id="modal-4" size="lg" :title="$t('externalTradeFair.circular_exhibition') + ' ' + $t('globalTrans.details')" hide-footer ok-only ok-variant="danger">
          <template #modal-title class="flex-grow-1">
            <span style="margin-top: 3px;">{{ $t('externalTradeFair.circular_exhibition') + ' ' + $t('globalTrans.details') }}</span>
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.tradeFairInfoManagement.subject, label_bn: i18n.messages.bn.tradeFairInfoManagement.subject, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '15%' } },
  { label_en: i18n.messages.en.externalTradeFair.country_city, label_bn: i18n.messages.bn.externalTradeFair.country_city, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '15%' } },
  ...
  { label_en: i18n.messages.en.globalTrans.action, label_bn: i18n.messages.bn.globalTrans.action, class: 'text-center', show: '1', order: 7, thStyle: { width: '20%' } }
]
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, '/portal/itf-circular-exhibition-list', params)
      if (result.success) {
        // this.dropdown = result.dropdown
        // this.userList = result.userDetails
        const listData = result.data.data.map((item, index) => {
          const country = this.$store.state.Portal.commonObj.countryList.find(obj => obj.value === parseInt(item.country_id))
          const customItem = {
    ...
    setServiceUrl (circularId) {
      this.$store.dispatch('Portal/setServiceUrl', {
        externalPanelServiceUrl: '/trade-fair-panel/exhibit-information-form?exb_circular_id=' + circularId,
        externalPanelCurrentServiceUrl: '/trade-fair-panel/exhibit-information-form?exb_circular_id=' + circularId
      })
