  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('approvalProcess.recommended') + ' ' + $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title"> {{ $t('approvalProcess.recommended') + ' ' + $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(remarks)="data">
                    <slot v-if="data.item.recommendations">
                      {{ data.item.recommendations[0].remarks}}
                    </slot>
                  </template>
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button class="moc-action-btn moc-recommend-btn" v-if="data.item.status === 2" :title="$t('globalTrans.recommended')" v-b-modal.modal-5 @click="recommended(data.item)">
                      <i class="ri-hand-coin-fill"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-approve-btn" :title="$t('globalTrans.approve')" v-b-modal.modal-6 @click="recommended(data.item)">
                      <i class="ri-shield-check-line"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-reject-btn" :title="$t('globalTrans.reject')" v-b-modal.modal-4 @click="recommended(data.item)">
                      <i class="ri-close-circle-line"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-recommend-btn" :title="$t('approvalProcess.re_recommendation')" v-b-modal.modal-5 @click="recommended(data.item)">
                      <i class="ri-hand-coin-fill"></i>
                    </b-button>
                  </template>
                </b-table>
    ...
    <b-modal id="modal-4" size="lg" :title="$t('globalTrans.reject')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <Reject :id="recommendedItemId" :item="item"/>
    </b-modal>
    <b-modal id="modal-6" size="lg" :title="$t('globalTrans.approve')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <ApproveTree v-if="service_id == 2" :id="recommendedItemId" hide-footer/>
      <Approve v-else  :id="recommendedItemId"  :item="item"/>
    </b-modal>
    <b-modal id="modal-5" size="lg" :title="$t('approvalProcess.recommendation')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <Recommended :id="recommendedItemId"  :item="item"/>
    </b-modal>
  </div>
</template>
<script>
import RestApi, { teaGardenServiceBaseUrl } from '@/config/api_config'
import { applicantRecommendedList, applicantToggle } from '../../api/routes'
import ModalBaseMasterList from '@/mixins/list'
import Approve from './Approve'
import Reject from './Reject'
import Recommended from './Recommended'
import ApproveTree from './ApproveTree'

export default {
  mixins: [ModalBaseMasterList],
 components: {
    Approve, Reject, Recommended, ApproveTree
  },
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'globalTrans.remarks', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '15%' } },
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, applicantRecommendedList, params)
      if (result.success) {
