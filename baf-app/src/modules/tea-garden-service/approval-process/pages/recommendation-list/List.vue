  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('approvalProcess.recommendation') + ' ' + $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title"> {{ $t('approvalProcess.recommendation') + ' ' + $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button class="moc-action-btn moc-recommend-btn" :title="$t('globalTrans.recommend')" v-b-modal.modal-5 @click="recommended(data.item)">
    ...
    <b-modal id="modal-5" size="lg" :title="$t('approvalProcess.recommendation')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <RecommendationTreeDisposal v-if="service_id == 2" :id="recommendedItemId" hide-footer/>
      <Recommendation v-else :id="recommendedItemId" hide-footer/>
    </b-modal>
  </div>
</template>
<script>
import RestApi, { teaGardenServiceBaseUrl } from '@/config/api_config'
import { applicantRecommendationList, applicantToggle } from '../../api/routes'
import ModalBaseMasterList from '@/mixins/list'
import Recommendation from './Recommendation'
import RecommendationTreeDisposal from './RecommendationTreeDisposal'

export default {
  mixins: [ModalBaseMasterList],
 components: {
    Recommendation, RecommendationTreeDisposal
  },
  data () {
    return {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('approvalProcess.recommendation') + ' ' + this.$t('approvalProcess.recommendation') : this.$t('approvalProcess.recommendation') + ' ' + this.$t('globalTrans.modify')
    },
    applicantList () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, applicantRecommendationList, params)
      if (result.success) {
