              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <!-- payment routes -->
                   <router-link class="moc-action-btn moc-payment-btn" :to="`payment/${data.item.application.id}?from=tea_garden_service.btb.tea_garden_application`" v-if="data.item.application.status === 1 && data.item.application.payment_status == 0 && data.item.application.service.has_payment" :title="$t('teaGardenBtriService.payment_for')">
                    ...
                    <router-link class="moc-action-btn moc-view-btn" :to="'tea-garden-application/'+data.item.id" :title="$t('globalTrans.view')">
                      <i class="ri-eye-line"></i>
                    </router-link>
                    <b-button class="moc-action-btn moc-edit-btn" v-if="data.item.application.status === 1" @click="editData(data.item)" :title="$t('globalTrans.edit')">
                      <i class="ri-pencil-fill"></i>
                    </b-button>
                    <router-link class="moc-action-btn moc-forward-btn" v-if="data.item.application.status === 6" :to="'tea-garden-application-certificate/'+data.item.id" :title="$t('globalTrans.certificate')">
                      <i class="ri-file-paper-2-line"></i>
                    </router-link>
                    <router-link class="moc-action-btn moc-forward-btn blinking" v-if="isWarningTime(data.item.application)" :to="'tea-garden-renew-application-form/'+data.item.id" :title="$t('globalTrans.renew')">
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.approvalProcess.app_id, label_bn: i18n.messages.bn.approvalProcess.app_id, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4 },
...
export default {
  ...
  methods: {
    editData (item) {
      if (this.authUser.user_type === 1) {
        if (item.application.application_type === 2) {
          this.$router.push(`/tea-garden-service/btb/tea-garden-renew-application-form/${item.application.parent.taggable.id}?id=${item.id}`)
        } else {
          this.$router.push(`/tea-garden-service/btb/tea-garden-application-form?id=${item.id}`)
        }
      } else {
        if (item.application.application_type === 2) {
    ...
    getFeeTypeName (id) {
      let feeType
      if (this.authUser.user_type === 1) {
        feeType = this.$store.state.TeaGardenService.commonObj.feeTypesList.find(item => item.value === id)
      } else {
        feeType = this.$store.state.ExternalUserService.teaGarden.commonObj.feeTypesList.find(item => item.value === id)
      }
    ...
    isWarningTime (application) {
      ...
      return ((toDate >= warningDate) && (application.status === 6) && !application.child)
    }
  }
}
</script>
