                ...
                <template v-slot:body>
                    ...
                    <form-wizard shape="circle" title="" subtitle="" :start-index.sync="activeIndex">
                        <tab-content :title="$t('teaGardenConfig.garden_info')" icon="ri-file-cloud-line" :before-change="tabSubmitGardenInfo" >
                            <GardenInfoForm :key="formData.id" :tabFormData.sync="formData" ref="garden_info_form" :status.sync="status"/>
                        </tab-content>
                        <tab-content :title="$t('teaGardenConfig.identity_info')" icon="ri-contacts-line"  :before-change="tabSubmitProprietorQuantity">
                            <ProprietorInfoForm :key="formData.proprietor_identity" :tabFormData="formData.proprietor_identity" ref="proprietor_info_form" :status.sync="status"/>
                        </tab-content>
                        <tab-content :title="$t('teaGardenConfig.landlease_info')" icon="ri-play-list-add-line" :before-change="tabSubmitLandOwnerShipInfo">
                           <LandOwnershipInfoForm :key="formData.land_ownership_info" :tabFormData.sync="formData.land_ownership_info" ref="land_ownership_form" :status.sync="status" />
                        </tab-content>
                        ...
                        <tab-content :title="$t('teaGardenConfig.witness_info')" icon="ri-list-settings-line" :before-change="tabSubmitWitnessInfo">
                            <WitnessInfoForm :key="formData.witness_info" :tabFormData="formData.witness_info" ref="witness_info_form" :status.sync="status" />
                        </tab-content>
                        <template slot="footer" slot-scope="props">
                            <div class="wizard-footer-right">
                                ...
                                <wizard-button
                                    ...
                                    title="Cancel"
                                >
                                    <router-link to="tea-garden-application" class="text-white"><i class="ri-close-circle-line"></i> {{ $t('globalTrans.cancel') }}</router-link>
                                </wizard-button>
                            </div>
                        </template>
                    </form-wizard>
                ...
                if (result.success) {
                    ...
                    if (this.status === 1) {
                        if (result.is_payment) {
                            ...
                            this.$router.push({ name: 'tea_garden_service.btb.payment', params: { id: baseTableId }, query: { from: 'tea_garden_service.btb.tea_garden_application' } })
                        } else {
                            this.$router.push({ name: 'tea_garden_service.btb.tea_garden_application' })
                        }
                    }
                }
            }
