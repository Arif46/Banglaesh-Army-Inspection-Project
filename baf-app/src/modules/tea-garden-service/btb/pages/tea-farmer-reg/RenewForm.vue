            ...
            <b-row>
                <b-col>
                    <form-wizard shape="circle" color="#005B5B" title="" subtitle="" :start-index.sync="activeIndex">
                        <tab-content :title="$t('teaGardenPanel.general_info')" icon="ri-home-4-line" :before-change="tabSubmitBasic" >
                            <div class="application-itmes">
                                 <BasicForm :key="key" :app_id="app_id" :basisFormData="application" :isShow="dataLoaded" :baseUrl="baseUrl" :isRenew="true" ref="basic_form"/>
                            </div>
                        </tab-content>
                        <tab-content :title="$t('teaGardenPanel.owner_info')" icon="ri-list-settings-line" :before-change="tabOwnerInfo">
                            <div class="application-itmes">
                                 <OwnerInfo :key="key" :app_id="app_id" :ownerInfoData="application.owner_info" :draft="draft" :isRenew="true" ref="owner_info_form"/>
                            </div>
                        </tab-content>
                        <template slot="footer" slot-scope="props">
                            <div class="wizard-footer-right">
                                ...
                                <wizard-button
                                    ...
                                    title="Cancel"
                                >
                                    <router-link to="/tea-garden-service/btb/tea-farmer-application" class="text-white"><i class="ri-close-circle-line"></i> {{ $t('globalTrans.cancel') }}</router-link>
                                </wizard-button>
                            </div>
                        </template>
                    </form-wizard>
...
export default {
  ...
  created () {
    if (this.$route.query.id) {
        ...
        this.getTeaFarmerData(this.$route.query.id)
    } else if (this.$route.params.parent_id) {
        this.isLoading = true
        this.getTeaFarmerData(this.$route.params.parent_id)
        this.app_id = 0
    } else {
  ...
  methods: {
    ...
    async getTeaFarmerData (appId) {
        const result = await RestApi.getData(teaGardenServiceBaseUrl, teaFarmerApplicationDetailsApi, { app_id: appId })
        if (result.success) {
    ...
    async tabOwnerInfo () {
        ...
        if (result.success) {
            if (this.draft) {
                this.$router.push({ path: '/tea-garden-service/btb/tea-farmer-application' })
            } else {
                if (result.is_payment) {
                    const baseTableId = result.base_table_id
                    this.$router.push({ name: 'tea_garden_service.btb.payment', params: { id: baseTableId }, query: { from: 'tea_garden_service.btb.received_application' } })
                } else {
                    this.$router.push({ name: 'tea_garden_service.btb.received_application' })
                }
            }
        }
    },
