<template>
  <div>
                  ...
                  <table class="payment-info">
                    <tbody>
                      <tr>
                        ...
                        <td><span> {{ $n(data.fee_amount) }}</span></td>
                      </tr>
                    </tbody>
                  </table>
                </b-card>
    ...
    <pre>{{ paymentTypeList[0].value }}</pre>
  </div>
</template>
<script>
  import RestApi, { teaGardenServiceBaseUrl } from '@/config/api_config'
  import { ValidationObserver, ValidationProvider } from 'vee-validate'
  import { applicationPaymentOffline, applicationPaymentOnline, applicationPaymentDetails } from '../../api/routes'
  ...
  export default {
    ...
    computed: {
      ...
      paymentTypeList: function () {
        if (this.isGardenAdmin || this.$store.state.Auth.authUser.user_type === 2) {
    ...
    methods: {
      async getData (id) {
          let result = null
          result = await RestApi.getData(teaGardenServiceBaseUrl, `${applicationPaymentDetails}/${id}`)
      ...
      async onlinePayment () {
        ...
        tmpPay = Object.assign(this.payment_info, { applicant_name: name, email: email, mobile: mobile })
        this.$store.dispatch('mutateCommonProperties', { loading: true })
          let result = null
          this.loading = true
          const loadingState = { loading: false, listReload: false }
