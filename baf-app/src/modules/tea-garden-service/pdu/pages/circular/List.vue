  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('teaGardenPduService.circular_prepare') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title"> {{ $t('teaGardenPduService.circular_prepare') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(total_application)="data">
                    <span>{{ $n(data.item.applications.length) }}</span>
                  </template>
                  <template v-slot:cell(total_candidate)="data">
                    <span>{{ $n(data.item.total_candidate) }}</span>
                  </template>
                  ...
                  <template v-slot:cell(action)="data">
                    <b-button v-if="data.item.status === 1" v-b-modal.modal-form variant=" iq-bg-success mr-1" size="sm" @click="editMethodInParent(data.item, 1)" class="action-btn edit" :title="$t('globalTrans.edit')"><i class="ri-pencil-fill"></i></b-button>
                    <router-link class="moc-action-btn moc-view-btn"
                     :to="{name: 'tea_garden_service.pdu.circular_view', params: { id: data.item.id }}"
                     :title="$t('globalTrans.view')">
                     <i class="ri-eye-line"></i>
                    </router-link>
                    <b-button v-if="data.item.status === 1" class="moc-action-btn moc-forward-btn" :title="$t('globalTrans.forward')" v-b-modal.modal-5 @click="forwarded(data.item)">
                      <i class="ri-arrow-go-forward-fill"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-publish-btn" v-b-modal.modal-8 :title="$t('globalTrans.approve')" @click="circularApprove(data.item)" v-if="data.item.status <= 2">
    ...
    <b-modal id="modal-8" size="lg" :title="$t('globalTrans.approve')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
...
import { circularList, circularToggle } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'teaGardenPduService.total_candidate', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '15%' } },
  ...
  computed: {
    ...
    pudCircularList () {
      return this.$store.state.TeaGardenService.commonObj.pduCircularList.filter(item => item.status === 1)
    },
    courseList: function () {
  ...
  methods: {
    ...
    getStatusInfo (item, type = 1) {
      ...
      if (item.status === 1) {
        ...
        statusText = this.$t('globalTrans.closed')
        statusClass = 'danger'
      }
    ...
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(3, teaGardenServiceBaseUrl, circularToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, circularList, params)
      if (result.success) {
