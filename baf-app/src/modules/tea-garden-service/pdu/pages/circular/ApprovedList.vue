<template>
    <div class="inner-section">
      <card>
        ...
        <template v-slot:searchHeaderTitle>
          <h4 class="card-title">{{ $t('teaGardenPduService.circular_approve') }} {{ $t('globalTrans.search') }}</h4>
        </template>
      ...
      <body-card>
        ...
        <template v-slot:headerTitle>
          <h4 class="card-title"> {{ $t('teaGardenPduService.circular_approve') }} {{ $t('globalTrans.list') }}</h4>
        </template>
                ...
                <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                  <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                    ...
                    <template v-slot:cell(action)="data">
                      ...
                      <b-button class="moc-action-btn btn-danger" :title="$t('globalTrans.closed')" @click="closedCircular(data.item)" v-if="data.item.status === 3">
                      ...
                      <router-link class="btn btn-sm btn-success" style="border-radius:20px;width: 114px" v-if="data.item.application_deadline >= today && data.item.status === 3"
                       :to="{name: 'tea_garden_service.pdu.course_application_form', query: { circular_id: data.item.id }}"
  ...
  import { circularApprovedList, circularClosed } from '../../api/routes'
  ...
  export default {
    ...
    data () {
      return {
        ...
        labelData: [
          ...
          { labels: 'teaGardenPduService.total_application', class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '15%' } },
    ...
    methods: {
      closedCircular (item) {
        window.vm.$swal({
          title: window.vm.$t('teaGardenPduService.closed_confirm_msg'),
          ...
          if (result.isConfirmed) {
            ...
            RestApi.deleteData(teaGardenServiceBaseUrl, `${circularClosed}/${item.id}`).then(response => {
              if (response.success) {
