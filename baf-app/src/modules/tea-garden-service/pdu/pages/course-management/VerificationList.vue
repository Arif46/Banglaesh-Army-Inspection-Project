  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('dealer.application') }} {{ $t('teaGardenPduService.verification') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('dealer.application') }} {{ $t('teaGardenPduService.verification') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { verificationListApi, applicantInfoVerifyApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, verificationListApi, params)
      if (result.success) {
    ...
    verify (item) {
      window.vm.$swal({
        title: window.vm.$t('teaGardenBtriService.verify_confirm_msg'),
        ...
        if (result.isConfirmed) {
          this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
          RestApi.deleteData(teaGardenServiceBaseUrl, `${applicantInfoVerifyApi}/${item.id}`).then(response => {
            if (response.success) {
