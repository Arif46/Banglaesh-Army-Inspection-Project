<template>
  ...
  import { paymentSuccess } from '../../../api/routes'
  ...
  export default {
    ...
    methods: {
      async getData (transId) {
          let result = null
          result = await RestApi.getData(teaGardenServiceBaseUrl, paymentSuccess, { transId: transId })
          if (result.success === 2) {
            //  route name get
            ...
            if (this.$store.state.Auth.authUser.user_type === 2) {
              ...
              routeName = 'tea_garden_service.pdu.course_application'
            }
