  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('teaGardenConfig.section_nm') }}</h4>
      </template>
        ...
        <b-row>
          ...
          >
            <template v-slot:label>
              {{ $t('teaGardenService.garden_name') }}
            </template>
          <b-form-select
              plain
    ...
    <body-card>
            ...
            <template v-slot:headerTitle>
              <h4 class="card-title">{{ $t('teaGardenConfig.section_entry_list') }} {{ $t('globalTrans.list') }}</h4>
            </template>
                ...
                <b-row>
                  <b-col md="12" class="table-responsive">
                    <slot v-if="columns.filter(item => item.show === '1').length > 0">
                      <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection"
                        head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty
                        bordered hover
                        ...
                        <template v-slot:cell(section)="data">
                          {{ getSectionsNameArray(data.item.details) }}
                        </template>
                        ...
                        <template v-slot:cell(action)="data">
                          ...
                          <b-button class="moc-action-btn" :variant="data.item.status === 2 ? ' moc-secondary-btn' : ' moc-assign-btn'"
                            :title="data.item.status === 2 ? $t('globalTrans.inactive') : $t('globalTrans.active')" @click="editMethodInParent(data.item,2)">
                    ...
                    <div class="pagination-wrapper mt-3" v-if="columns.filter(item => item.show === '1').length > 0">
                      <b-pagination v-model="pagination.currentPage" :perPage="search.limit" :total-rows="pagination.totalRows"
                        @input="searchData" />
                    </div>
                  </b-col>
                </b-row>
...
import { sectionMainListApi, sectionMainToggleApi } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'teaGardenConfig.section', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '15%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('teaGardenConfig.section_nm2') + ' ' + this.$t('globalTrans.entry') : this.$t('teaGardenConfig.section_nm2') + ' ' + this.$t('globalTrans.modify')
    },
    currentLocale () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(3, teaGardenServiceBaseUrl, sectionMainToggleApi, item)
      } else if (type === 3) {
      this.details(item)
    }
    },
    ...
    const result = await RestApi.getData(teaGardenServiceBaseUrl, sectionMainListApi, params)
    if (result.success) {
    ...
    getSectionsNameArray (data) {
      var sectionName = []
      if (Array.isArray(data)) {
        data.forEach(arrItem => {
          if (this.$i18n.locale === 'bn') {
            sectionName.push(arrItem.section_bn)
          } else if (this.$i18n.locale === 'en') {
            sectionName.push(arrItem.section_en)
          }
        })
      }
