    ...
    <body-card>
      ...
      <template v-slot:body>
        ...
        <div class="cover-letter-wrapper" style="text-align: justify">
          ...
          <p style="line-height: 2rem;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ $t('teaGardenBtriService.greeting_heading') }} <span class="dynamic-line-wrapper">{{ getGardenName(appDetails.garden_id) }}</span> {{ $t('teaGardenBtriService.greeting_2nd') }}
            <span style="min-width: 23px" class="dynamic-line-wrapper ml-2"> {{ $n(appDetails.total_packets) }}
            </span> {{ $t('teaGardenBtriService.packets') }} {{ $t('teaGardenBtriService.dolomite') }} <span :key="'item-' +index" v-if="index < (appDetails.details.length - 1)">,</span>
