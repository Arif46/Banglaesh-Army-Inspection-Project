    ...
    <body-card>
      ...
      <template v-slot:body>
        ...
        <div class="cover-letter-wrapper" style="text-align: justify">
          ...
          <p>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;{{ $t('teaGardenBtriService.greeting_heading') }} <span class="dynamic-line-wrapper">{{ getGardenName(appDetails.garden_id) }}</span> {{ $t('teaGardenBtriService.greeting_2nd_organic_compost') }}
          <template v-if="appDetails.details">
            <slot v-for="(item, index) in appDetails.details">
              <span :key="index" style="min-width: 23px" class="dynamic-line-wrapper ml-2"> {{ $n(item.packets) }}</span> {{ currentLocale === 'bn' ? 'টি' : '' }} {{ getFertilizerTypeName(item.fertilizer_type) }} <span :key="'item-' +index" v-if="index < (appDetails.details.length - 1)">,</span>
            </slot>
          </template>
            ...
            <table class="table table-sm table-bordered section-tree-view-table">
                <thead>
                    <tr>
                        ...
                        <th class="text-center">{{ $t('teaGardenBtriService.fertilizer_type') }}</th>
                ...
                <tbody>
                    <slot v-for="(item, index) in appDetails.details">
                      <tr :key="index">
                        ...
                        <td class="text-center"><span v-if="item.fertilizer_type">{{ getFertilizerTypeName(item.fertilizer_type) }}</span></td>
