  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('teaGardenBtriService.forwarded_application') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title"> {{ $t('teaGardenBtriService.forwarded_application') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-info">{{$t('globalTrans.forwarded_to')}} {{ ($i18n.locale === 'bn') ? data.item.forwards[0].receiver_name_bn : data.item.forwards[0].receiver_name }}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <!-- <b-button class="btn action-btn status btn- btn-success btn-sm" :variant="data.item.status === 2 ? ' btn-danger' : ' btn-success'" :title="data.item.status === 2 ? $t('globalTrans.inactive') : $t('globalTrans.active')" size="sm" @click="assign(data.item)" v-if="data.item.status === 2">
                      <i :class="data.item.status === 2  ? 'ri-toggle-line' : 'ri-toggle-fill'"></i>
                    </b-button> -->
                    <b-button class="moc-action-btn moc-forward-btn" :title="$t('globalTrans.forward')" v-b-modal.modal-5 @click="forwarded(data.item)">
                      <i class="ri-arrow-go-forward-fill"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-return-btn" v-b-modal.modal-4 :title="$t('globalTrans.return')" @click="returnApplication(data.item)" v-if="!isGardenAdminCheckGardenId() && (data.item.status > 1 && data.item.status < 5)">
                      <i class="ri-reply-all-fill"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-sample-verify-btn" :title="$t('teaGardenBtriService.sample_verify')" @click="sampleVerify(data.item)">
...
import { applicationSampleVerify, forwardedList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    sampleVerify (item) {
      window.vm.$swal({
        ...
        if (result.isConfirmed) {
          ...
          RestApi.deleteData(teaGardenServiceBaseUrl, `${applicationSampleVerify}/${item.id}`).then(response => {
            if (response.success) {
              window.vm.$toast.success({
                title: window.vm.$t('teaGardenBtriService.verification_success_msg'),
    ...
    async loadData () {
      ...
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(teaGardenServiceBaseUrl, forwardedList, params)
      if (result.success) {
