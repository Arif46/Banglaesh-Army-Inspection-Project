  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('teaGardenBtriService.report_prepare_application') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title"> {{ $t('teaGardenBtriService.report_prepare_application') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button class="moc-action-btn moc-payment-receipt-btn" v-b-modal.modal-6 :title="$t('externalLrcpn.payment_receipt')" @click="reportPrepare(data.item)" v-if="(data.item.status >= 2) && (data.item.payment)">
                      <i class="ri-file-shield-2-line"></i>
                    </b-button>
                    <b-button class="moc-action-btn moc-upload-btn" v-b-modal.modal-7 :title="$t('teaGardenBtriService.report_upload')" @click="reportPrepare(data.item)">
    ...
    <b-modal id="modal-7" size="lg" :title="$t('teaGardenBtriService.report_prepare')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <ReportPrepare :id="item.id" :item="item"/>
    </b-modal>
...
import { reportPrepareList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, reportPrepareList, params)
      if (result.success) {
