<template>
    <div class="inner-section">
      <card>
        ...
        <template v-slot:searchHeaderTitle>
          <h4 class="card-title">Search Offer</h4>
        </template>
      ...
      <body-card>
        ...
        <template v-slot:headerAction>
          <!-- <a href="javascript:" class="btn-add" @click="resetId" v-b-modal.modal-form><i class="ri-add-fill"></i> {{ $t('globalTrans.add_new') }}</a> -->
                <router-link to="specification-verify-form" class="btn-add"><i class="ri-add-fill"></i> {{ $t('globalTrans.add_new') }}</router-link>
        </template>
                ...
                <slot v-if="columns.filter(item => item.show === '1').length > 0">
                  <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                    ...
                    <template v-slot:cell(item)="data">
                      {{ data.item?.items_name?.item_name }}
                    </template>
                    <template v-slot:cell(action)="data">
                      <router-link class="action-btn edit" :title="$t('globalTrans.edit')" :to="{ path: 'specification-verify-form?id=' + data.item.id, query: { item: data.item }}"  size="sm">
  ...
  import { specificationVerifyList } from '../../api/routes'
  ...
  const defaultColumn = [
    ...
    { label_en: i18n.messages.en.specification.item_name, label_bn: i18n.messages.bn.specification.item_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '25%' } },
    { label_en: i18n.messages.en.specification.supplier_name, label_bn: i18n.messages.bn.specification.supplier_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '25%' } },
  ...
  export default {
    ...
    computed: {
      ...
      formTitle () {
        return 'Specification Veify Entry'
      },
      detailsTitle () {
        return 'Offer Vetting Details'
      },
      pageOptions () {
    ...
    methods: {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(InspectionServiceBaseUrl, specificationVerifyList, params)
        if (result.success) {
