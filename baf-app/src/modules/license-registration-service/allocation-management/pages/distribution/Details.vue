            ...
            <b-form @submit.prevent="handleSubmit()" @reset.prevent="reset">
                <table style="width:100%" class="text-center">
                    <tr>
                        ...
                        {{$t('globalTrans.sl_no')}}- {{ $n(items.id) }}
                        </th>
                    </tr>
                    <tr class="text-center" style="margin-top:20px;">
                        ...
                        <td>{{$t('globalTrans.date')}} : {{ items.distribution_date | dateFormat }}</td>
                    </tr>
                    <tr>
                        <td colspan="3">
                            <table style="width:100%;margin-top:10px" border="1">
                                ...
                                ;width:30%;">&nbsp;{{$t('allocation_management.all_letter_no')}} - {{ $n(items.allocation.allocation_letter_id) }}</td>
                                    <td style="width:20%;">{{$t('globalTrans.date')}} : {{ items.allocation.letter_date | dateFormat }}</td>
                                </tr>
                                <tr class="tboder">
                                    <td class="tboder">
                                    ...
                                    <td colspan="2" class="tboder text-left">
                                       ...
                                       <p> &nbsp;{{$t('allocation_management.mr_no')}} {{ items.transaction_no }}</p>
                                       <p> &nbsp;{{$t('allocation_management.bank_name')}} {{ items.bank_name }} {{ ' , ' }} {{ items.branch_name }}</p>
                                       <p> &nbsp;{{$t('allocation_management.warehouse')}} {{$t('globalTrans.name')}}: {{ getWareName(items.warehouse_id) }}</p>
                                    </td>
                                </tr>
                            </table>
                        </td>
                ...
                <table style="width:100%;margin-top:10px" border="1" class="text-center">
                    ...
                    <tr class="tboder" v-for="(detail,index) in items.allocation.details" :key="detail.id">
                ...
                <table style="width:100%;margin-top:10px;margin-bottom:10px;" border="1" class="text-center">
                    ...
                    <template v-if="items.details.length > 0">
                        <tr class="tboder" v-for="(item, index) in items.details" :key="index">
                            <td class="tboder" style="padding:15px">{{ getWareName(items.warehouse_id) }}</td>
                            <td class="tboder">{{ items.distribution_date | dateFormat }}</td>
...
export default {
    ...
    created () {
      this.paidAllocationDetails = this.items.allocation.details
    },
    mounted () {
        // core.index()
    },
    ...
    computed: {
        ...
        totalTax () {
            return this.items.allocation.details.reduce((sum = 0, item) => sum + item.tax_amount, 0)
        },
        allTotal () {
            return this.items.allocation.details.reduce((sum = 0, item) => sum + item.tax_amount + item.selling_amount, 0)
        }
    },
    methods: {
        empty (str) {
