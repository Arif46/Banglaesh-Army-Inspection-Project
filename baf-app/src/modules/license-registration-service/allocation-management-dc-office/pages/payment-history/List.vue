  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('globalTrans.payment') }} {{ $t('globalTrans.search') }}</h4>
      </template>
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <div v-if="isResionalOffice">
                      <span class="badge badge-danger" v-if="data.item.is_paid == 0">{{$t('globalTrans.process')}}</span>
                      <span class="badge badge-primary" v-if="data.item.is_paid == 1">{{$t('allocation_dc_office.processing')}}</span>
                      <span class="badge badge-success" v-if="data.item.is_paid == 2">{{$t('globalTrans.approved')}}</span>
                    </div>
                    <div v-else>
                      <span class="badge badge-danger" v-if="data.item.is_paid == 0">{{$t('license_management.pending')}}</span>
                      <span class="badge badge-primary" v-if="data.item.is_paid == 1">{{$t('globalTrans.received')}}</span>
                      <span class="badge badge-success" v-if="data.item.is_paid == 2">{{$t('globalTrans.approved')}}</span>
                    </div>
                  </template>
                  <template v-slot:cell(action)="data">
                   <b-button v-b-modal.challan-view   class="action-btn btn-info edit"  @click="challanView(data.item)" :title="$t('globalTrans.payment') +' '+ $t('globalTrans.details')"><i class="ri-window-2-line"></i></b-button>
                  </template>
                </b-table>
              </slot>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      viewTitle () {
        return this.$t('allocation_dc_office.payment') + ' ' + this.$t('globalTrans.details')
      },
      challanTitle () {
    ...
    methods: {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, paymentList, params)
        if (result.success) {
