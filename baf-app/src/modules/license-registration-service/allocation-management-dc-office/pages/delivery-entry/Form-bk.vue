...
<script>
  ...
  import { getItemByWarehouse, distributionDistribute } from '../../api/routes'
  export default {
    name: 'Form',
    props: ['item'],
    data () {
      ...
      if (this.item) {
        ...
        this.categoryList = []
         this.item.items.forEach(item => {
           this.itemList.push(this.getItemCategory(item.item_category_id))
        const reqItem = {
    ...
    watch: {
      'stockItemData.item_category_id': function (newVal, oldVal) {
        if (newVal !== oldVal && newVal !== 0) {
          ...
          this.allocatedQt = parseInt(askedItem.quantity)
          this.getWareItems(newVal)
        }
      },
    ...
    methods: {
      ...
      async addItem () {
        ...
        if (this.qtExced === false) {
          ...
          const newReqQt = parseInt(reqItemAvl.quantity) + parseInt(this.stockItemData.quantity)
      ...
      async getWareItems (newVal) {
        const params = {
          warehouse_id: this.warehouse_id,
          item_category_id: newVal
        }
        this.loading = true
