...
const exportPdfDetails = async (baseUrl, uri, orgId, items, challanItems, vm) => {
  try {
        ...
        if (i18n.locale === 'bn') {
          ...
          const allRowsHead1 = [
            [
              { text: vm.$t('allocation_dc_office.delivery') + '  ' + vm.$t('allocation_dc_office.challan'), alignment: 'center', style: 'thh', colSpan: '2' },
              {}
            ]
          ]
          allRowsHead1.push([
            { text: vm.$t('globalTrans.fiscal_year') + ' : ' + vm.getFiscalYear(items.fiscal_year_id), alignment: 'left', style: 'thh' },
            { text: vm.$t('allocation_management.allotment_count') + ' : ' + vm.$n(items.allotment_count ? items.allotment_count : items.allotment_count_id), alignment: 'right', style: 'thh' }
          ])
          allRowsHead1.push([
            { text: vm.$t('allocation_dc_office.delivery_date') + ' : ' + dateFormat(items.allotment_date), alignment: 'left', style: 'thh' },
            { text: vm.$t('allocation_dc_office.delivery_order_no') + ' : ' + vm.$n(items.delivery_order_no), alignment: 'right', style: 'thh' }
          ])
          allRowsHead1.push([
            { text: vm.$t('allocation_dc_office.dc_office_name') + ' : ' + vm.getDistrictName(items.district_id), alignment: 'center', style: 'thh', colSpan: '2' },
            {}
          ])
          allRowsHead1.push([
            { text: vm.$t('allocation_management.regional_office') + ' : ' + vm.getOfficeName(items.regional_office_id), alignment: 'left', style: 'thh' },
            { text: vm.$t('allocation_dc_office.warehouse_name') + ' : ' + vm.getWareName(items.warehouse_id), alignment: 'right', style: 'thh' }
          ])
          allRowsHead1.push([
          ...
          const lastwidths = ['5%', '15%', '15%', '10%', '10%', '18%', '15%', '12%']
            pdfContent.push({
              style: 'tableExample',
              table: {
                widths: lastwidths,
            ...
            const footerHead = [
              [
                { text: '', alignment: 'left', style: 'margin40' },
                { text: '', alignment: 'left', style: 'margin40' }
              ]
            ]
            footerHead.push([
