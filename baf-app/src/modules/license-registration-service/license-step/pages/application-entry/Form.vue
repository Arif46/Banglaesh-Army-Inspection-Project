          ...
          <b-form  @submit.prevent="handleSubmit(saveFormData)" @reset.prevent="reset" v-if="tindex > 0">
            <b-overlay :show="saveloading">
              ...
              <b-row v-if="prevManualData.hasManual === true">
                ...
                <b-col lg="6" sm="6" md="6">
                    <ValidationProvider name="Issue Date" vid="issue_date">
                        ...
                        >
                        <template v-slot:label>
                          {{ $t('externalLrcpn.issue_date')}} <span class="text-danger">*</span>
                        </template>
                        <b-form-input
                            id="datepicker"
                ...
                <b-col lg="6" sm="6" md="6">
                    <ValidationProvider name="Expire Date" vid="expire_date">
                        ...
                        >
                        <template v-slot:label>
                          {{ $t('externalLrcpn.expire_date')}} <span class="text-danger">*</span>
                        </template>
                        <b-form-input
                            id="datepicker"
                            placeholder="yyyy-mm-dd"
                  ...
                  <b-col lg="6" sm="12" v-if="locationShow">
                    <ValidationProvider name="Area Type" vid="area_type_id">
                        <b-form-group
                            ...
                            >
                            <template v-slot:label>
                                {{$t('organogram.area_type')}} <span class="text-danger">*</span>
                            </template>
                            <b-form-select
                            plain
                  ...
                  <b-col lg="6" sm="12" v-if="PauroshobaItemShow || UnionItemShow">
                      <ValidationProvider name="Upazila" vid="upazila_id" rules="required|min_value:1">
                          <b-form-group
                              ...
                              >
                              <template v-slot:label>
                              {{ $t('globalTrans.upazilla')}} <span class="text-danger">*</span>
                              </template>
                              <b-form-select
                              plain
              ...
              <b-row>
                <b-col lg="6" sm="12" v-if="officeCategoryShow">
                  <ValidationProvider name="Office Category" vid="office_cat_id" rules="required|min_value:1">
                      <b-form-group
                          ...
                          >
                          <template v-slot:label>
                              {{ $t('userManagement.office_category') }} <span class="text-danger">*</span>
                          </template>
                          <b-form-select
                          plain
                ...
                <b-col lg="6" sm="12">
                  <ValidationProvider name="Office" vid="type" rules="required|min_value:1">
                      <b-form-group
                          class="row"
                          label-cols-sm="12"
                          label-for="type"
                          ...
                          >
                          <template v-slot:label>
                              {{ $t('li_step.application_type') }} <span class="text-danger">*</span>
                          </template>
                          <b-form-select
                          plain
                          v-model="type"
                ...
                <b-col lg="6" sm="12" v-if="type === 2 && prevManualData.hasManual === false">
                  <ValidationProvider name="Office" vid="application_id">
                    <b-form-group
                        class="row"
                        label-cols-sm="12"
...
<!--                          </div>-->
                          <div class="row">
                            <div class="col text-right">
                              ...
                              <router-link v-if="formActions.isCancel && index === totalTab" to="application-entry" :class="'btn btn-danger mr-1'">{{ $t('globalTrans.cancel') }}</router-link>
...
import { userEntryForm, userEntryStore, loadOldFormData } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      is_disable: false,
      applicationTypeList: [
        { value: 1, text: this.$i18n.locale === 'bn' ? 'নতুন' : 'New' },
        { value: 2, text: this.$i18n.locale === 'bn' ? 'নবায়ন করুন' : 'Renew' }
      ],
      type: 0,
      officeCategoryList: [
  ...
  created () {
    ...
    this.gridSearch.org_id = this.$store.state.Auth.authUser.org_id
  ...
  methods: {
      ...
      if (typeof formData === 'undefined') {
        ...
        RestApi.getData(licenseRegistrationServiceBaseUrl, userEntryForm, params).then(response => {
          if (response.conditionData !== '') {
            if (response.conditionData.condition_type === 'notice') {
    ...
    async saveFormData () {
      ...
      this.sInfo.application_id = parseInt(this.application_id)
      this.officeData.type = this.type
      ...
      if (result.success) {
        ...
        this.$router.push('/license-registration-service/license-step/application-entry')
      } else {
    ...
    getOfficeListforOfcCategory () { // only for those org who have Office Category from Service Master
      const officeList = this.$store.state.CommonService.commonObj.officeList.filter(item => item.status === 1)
      this.officeList = []
      if (this.officeData.office_cat_id === 1) {
        return officeList.filter(item => item.division_id === this.officeData.division_id)
      }
      if (this.officeData.office_cat_id === 2) {
        return officeList.filter(item => item.district_id === this.officeData.district_id)
      }
      if (this.officeData.office_cat_id === 3) {
        return officeList.filter(item => item.upazilla_id === this.officeData.upazilla_id)
      }
      return officeList
    },
    checkOfficeExist (serviceId) {
      const service = this.$store.state.LicenseRegistrationService.commonObj.serviceNamesList.find(item => (item.value === serviceId))
    ...
    loadApplicationData () {
      const formData = this.$store.state.LicenseRegistrationService.commonObj.serviceStepForm.find(item => item.service_id === this.gridSearch.service_id && item.form_type === 2)
      if (typeof formData !== 'undefined') {
        const formId = formData.form_id
        const params = {
          form_id: formId,
