      ...
      <template v-slot:body>
        <ValidationObserver ref="searchForm" v-slot="{ handleSubmit }">
          <b-form  @submit.prevent="handleSubmit()" >
                ...
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                  <ValidationProvider name="Application ID" vid="application_id" rules="required">
                    ...
                    <template v-slot:label>
                      ...
                      <b-form-input
                        id="application_id"
                        v-model="applicationDatas.application_id"
            ...
            <b-overlay :show="unitLoad" v-if="showData">
              <template>
...
import { stepFormDetailsData, licenseTransferStore, applicationInfoApi } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    searchApplication () {
      if (this.applicationDatas.application_id === '') {
      ...
      this.unitLoad = true
        ...
        this.loading = true
        RestApi.getData(licenseRegistrationServiceBaseUrl, `${applicationInfoApi}/${this.applicationDatas.application_id}`).then(response => {
        if (response.success) {
          this.showData = true
          this.step_id = this.getGridStep(response.data.service_id).step_id
          this.gridSearch.org_id = parseInt(response.data.org_id)
          this.gridSearch.service_id = parseInt(response.data.service_id)
          this.stepList = this.getServiceStep(parseInt(response.data.service_id))
          this.checkOfficeExist(parseInt(response.data.service_id))
          this.loadFrom(parseInt(response.data.service_id))
        } else {
          this.$toast.error({
            title: 'Error',
    ...
    loadFrom (serviceId) {
      ...
      if (typeof formData === 'undefined') {
        ...
        const params = {
          ...
          applicationID: parseInt(this.applicationDatas.application_id)
        }
    ...
    async saveFormData () {
      ...
      this.sInfo.app_id = parseInt(this.applicationDatas.application_id)
      const formData = {
        data: sData,
      ...
      let result = null
      this.unitLoad = true
      result = await RestApi.postData(licenseRegistrationServiceBaseUrl, `${licenseTransferStore}/${this.table_id}`, formData)
      this.unitLoad = false
      if (result.success) {
        this.$toast.success({
    ...
    checkOfficeExist (serviceId) {
      ...
      if (service.office_type_id === null) {
        ...
        this.officeData.office_id = service.office_id
      }
    }
  }
}
