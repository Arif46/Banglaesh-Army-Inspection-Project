  ...
  <b-container fluid>
    <iq-card>
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('li_step.numerical_report') }}</h4>
      </template>
...
import { numericalReportList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    loadData () {
      ...
      RestApi.getData(licenseRegistrationServiceBaseUrl, numericalReportList, params).then(response => {
        if (response.success) {
