  ...
  <b-container fluid>
    <iq-card>
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('li_step.withdraw_list') }}</h4>
      </template>
    ...
    <b-modal id="modal-4" size="xl" :title="formTitle" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Form :id="editItemId" :key="editItemId" :formType='2'/>
    </b-modal>
  </b-container>
</template>
<script>
...
import { licenseWithdrawList } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
       return this.$t('li_step.withdraw')
    },
    columns () {
      const labels = [
  ...
  methods: {
    ...
    loadData () {
      ...
      RestApi.getData(licenseRegistrationServiceBaseUrl, licenseWithdrawList, params).then(response => {
        if (response.success) {
