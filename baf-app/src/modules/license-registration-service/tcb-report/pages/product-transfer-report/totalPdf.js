...
const exportPdfDetails = async (baseUrl, uri, orgId, vm, search, TotalData) => {
  try {
    const districts = TotalData
    const items = TotalData[0].items
    districts.forEach(district => {
        ...
        allRowsHead1.push([
          ...
          { text: vm.$t('tcb_report.total_product_transfer_report'), alignment: 'center', style: 'thh' },
          { text: '' + '' + '', alignment: 'left', style: 'thh' }
        ])
        ...
        widthone.push('5%')
          ...
          secondRow.push({ text: '', style: 'tablecell' })
          items.map((item) => {
            secondRow.push({ text: vm.$t('tcb_report.allocated_amount'), style: 'tablecell' })
          ...
          districts.map((district, index) => {
            ...
            if (district.report_items.length > 0) {
                district.report_items.map((item, itemIn) => {
                    looprow.push({ text: vm.$n(item.askQty), style: 'tablecell' })
                    looprow.push({ text: vm.$n(0), style: 'tablecell' })
                    looprow.push({ text: vm.$n(item.disQty), style: 'tablecell' })
                    looprow.push({ text: vm.$n((item.askQty) - (item.disQty)), style: 'tablecell' })
                })
            } else {
                items.map((item) => {
                    looprow.push({ text: vm.$n(0), style: 'tablecell' })
