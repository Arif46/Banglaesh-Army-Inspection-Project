         ...
         <b-row>
          <b-col xs="12" sm="12" md="4" lg="4" xl="4">
            ...
            >
              ...
              <b-form-select
                ...
                v-model="search.company_division_id"
              >
          ...
          <b-col xs="12" sm="12" md="4" lg="4" xl="4">
              ...
              <b-form-select
                ...
                v-model="search.company_district_id"
              >
                <template v-slot:first>
                  <b-form-select-option :value='0'>{{$t('globalTrans.select')}}</b-form-select-option>
                </template>
          ...
          <b-col xs="12" sm="12" md="4" lg="4" xl="4">
              ...
              <b-form-select
                ...
                :options="upazilaList"
                v-model="search.company_upazilla_id"
              >
                <template v-slot:first>
                  <b-form-select-option :value='0'>{{$t('globalTrans.select')}}</b-form-select-option>
                </template>
              ...
              <slot v-if="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-primary" v-if="data.item.status == 0">{{$t('license_management.pending')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <router-link class="action-btn active view" variant=" iq-bg-success mr-1" :title="$t('globalTrans.forward')" :to="`/license-registration-service/license-management/forward?stepId=${data.item.status}&applications=${JSON.stringify([data.item.id])}`"><i class="ri-send-plane-fill"></i></router-link>
                  </template>
                </b-table>
              </slot>
...
import { tcbApplicationList } from '../../api/routes'
...
export default {
  ...
  watch: {
    ...
    'search.company_division_id': function (newVal, oldVal) {
      this.districtList = this.getDistrictList(newVal)
    },
    'search.company_district_id': function (newVal, oldVal) {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, tcbApplicationList, params)
      if (result.success) {
