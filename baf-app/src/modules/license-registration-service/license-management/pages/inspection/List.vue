<template>
  <div class="inner-section">
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-primary">{{ $t('license_management.inspection') }} {{ (data.item.inspection_status ? $t('globalTrans.completed') : $t('license_management.pending')) }}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <router-link v-if="data.item.inspection_status === 1" class="action-btn active view" variant=" iq-bg-success mr-1" :title="$t('globalTrans.forward')" :to="`/license-registration-service/license-management/forward?stepId=${data.item.status}&applications=${JSON.stringify([data.item.id])}`"><i class="ri-send-plane-fill"></i></router-link>
                    <button v-if="!data.item.inspection_status" class="action-btn btn-success active view" v-b-modal.modal-inspection @click="setInspection(data.item.id)" variant=" mr-1" :title="$t('license_management.inspection')" ><i class="ri-file-line"></i></button>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-inspection" size="lg" :title="formTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Form :id="editItemId"/>
    </b-modal>
  </div>
</template>
...
import { tcbInspectionList } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return this.$t('license_management.inspection')
    }
  },
  methods: {
    setInspection (id) {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, tcbInspectionList, params)
      if (result.success) {
