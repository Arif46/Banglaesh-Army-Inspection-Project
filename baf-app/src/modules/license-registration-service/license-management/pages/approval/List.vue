              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    <span v-if="data.item.approval_status" class="badge badge-primary">{{$t('license_management.approved')}} {{$t('globalTrans.completed')}}</span>
                    <span v-else class="badge badge-primary">{{$t('license_management.approved')}} {{$t('license_management.pending')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <router-link v-if="data.item.approval_status === 1" class="action-btn active view" variant=" iq-bg-success mr-1" :title="$t('globalTrans.forward')" :to="`/license-registration-service/license-management/forward?stepId=${data.item.status}&applications=${JSON.stringify([data.item.id])}`"><i class="ri-send-plane-fill"></i></router-link>
                    <button v-if="!data.item.approval_status" class="action-btn active view mt-1" @click="docVerify(data.item.id)" variant=" iq-bg-success mr-1" :title="$t('license_management.approved')" ><i class="ri-check-fill"></i></button>
                    <button @click="setReject(data.item.id)" v-if="!data.item.approval_status" v-b-modal.modal-form class="action-btn btn-danger active view mt-1" variant=" iq-bg-success mr-1" :title="$t('license_management.resubmit_reject')" ><i class="ri-close-line"></i></button>
                  </template>
                </b-table>
              </slot>
...
import { approvalList, approvalStatus } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return this.$t('license_management.review')
    }
  },
  methods: {
    setReject (id) {
    ...
    docVerify (id) {
      window.vm.$swal({
        title: window.vm.$t('license_management.approvalStatus'),
    ...
    docVerifyStatus (id) {
        ...
        RestApi.deleteData(licenseRegistrationServiceBaseUrl, `${approvalStatus}/${id}`).then(response => {
          if (response.success) {
            window.vm.$toast.success({
              title: this.$t('globalTrans.success'),
              message: 'Application Approved Success.',
              color: '#D6E09B'
            })
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, approvalList, params)
      if (result.success) {
