  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('step_assign.step_assign') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('step_assign.service_steps_list') }}</h4>
      </template>
...
import { stepAssignList } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('step_assign.step_assign') + ' ' + this.$t('globalTrans.entry') : this.$t('step_assign.step_assign') + ' ' + this.$t('globalTrans.modify')
    },
    orgList () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, stepAssignList, params)
      if (result.success) {
