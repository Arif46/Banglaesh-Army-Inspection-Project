  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('component_settings.ekpay_manage') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('component_settings.ekpay_manage') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { ekpayList, ekpayToggleStatus } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'service_name.api_type', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '25%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('component_settings.ekpay_manage') + ' ' + this.$t('globalTrans.entry') : this.$t('component_settings.ekpay_manage') + ' ' + this.$t('globalTrans.modify')
    },
    orgList () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(7, licenseRegistrationServiceBaseUrl, ekpayToggleStatus, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, ekpayList, params)
      if (result.success) {
        const listData = result.data.data.map((item, index) => {
          ...
          const apiObj = this.apiNList.find(service => service.value === parseInt(item.api_type))
          const orgData = {}
          ...
          const apiData = {}
            if (typeof apiObj !== 'undefined') {
              apiData.api_type_en = apiObj.text_en
              apiData.api_type_bn = apiObj.text_bn
            } else {
              apiData.api_type_en = ''
              apiData.api_type_bn = ''
            }
          return Object.assign({}, item, { serial: index }, orgData, serviceData, apiData)
        })
