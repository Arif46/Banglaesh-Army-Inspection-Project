  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('org_notes.org_notes') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('org_notes.org_notes') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { orgNotesList, orgNotesToggleStatus } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'globalTrans.note', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '25%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('org_notes.org_notes') + ' ' + this.$t('globalTrans.entry') : this.$t('org_notes.org_notes') + ' ' + this.$t('globalTrans.modify')
    },
    orgList () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(7, licenseRegistrationServiceBaseUrl, orgNotesToggleStatus, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, orgNotesList, params)
      if (result.success) {
