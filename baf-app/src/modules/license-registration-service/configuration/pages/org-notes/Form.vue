    ...
    <b-overlay :show="loading">
      <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off"  enctype="multipart/form-data">
        <b-row>
          ...
          <b-col xs="12" sm="12" md="6">
            <ValidationProvider name="Note (En)" vid="note" rules="required" v-slot="{ errors }">
              <b-form-group
                label-for="note">
                <template v-slot:label>
...
import { orgNotesStore, orgNotesUpdate } from '../../api/routes'
export default {
  name: 'Form',
  props: ['id'],
  data () {
  ...
  methods: {
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(licenseRegistrationServiceBaseUrl, `${orgNotesUpdate}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(licenseRegistrationServiceBaseUrl, orgNotesStore, this.form)
      }
