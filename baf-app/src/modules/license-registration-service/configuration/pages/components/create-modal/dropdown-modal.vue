    ...
    data () {
        return {
            ...
            componentListDropdown: [
                ...
                { value: '1', text: 'Common Dropdown' },
                { value: '2', text: 'Licence & Registration' }
            ],
            formTitle: '',
            curStore: '',
            drpList: []
        ...
        'dropdownFields.component_id': function (newVal, oldVal) {
            if (newVal !== oldVal) {
                if (newVal === '0') {
                    ...
                    this.dropdownFields.dropdown_options = []
                    this.customDropdownStatus = false
                    this.commonDropdownStatus = true
                    this.licenseStatus = false
                    ...
                    this.curStore = this.$store.state.LicenseRegistrationService.commonObj
                    this.drpList = Object.entries(this.curStore).filter(([key, value]) => (key !== 'hasDropdownLoaded' && key !== 'activeIntactive' && key !== 'listReload' && key !== 'loading' && key !== 'perPage')).map((item, index) => {
                        const textValue = item[0].split(/(?=[A-Z])/).join(' ')
                        return { value: item[0], text: textValue }
                    })
                }
