...
import { stepFormList } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    editMethodInParent (item, type) {
      ...
      if (type === 1) {
        this.$router.push({ path: '/license-registration-service/configuration/create-form', query: { id: itemId } })
      } else if (type === 3) {
        this.$router.push({ path: '/license-registration-service/configuration/step-form-view', query: { id: itemId } })
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(licenseRegistrationServiceBaseUrl, stepFormList, params)
      if (result.success) {
