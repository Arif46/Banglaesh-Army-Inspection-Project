  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('dashboard.program_type') }}</h4>
      </template>
          ...
          <b-col sm="4">
            ...
            >
              <template v-slot:label>
                {{ $t('dashboard.program_type') }}
              </template>
              <b-form-input
                id="program_type"
                type="text"
                v-model="search.program_type"
              ></b-form-input>
            </b-form-group>
          </b-col>
          <b-col sm="4">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('dashboard.program_type') + ' ' + $t('globalTrans.list') }}</h4>
      </template>
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'dashboard.program_type', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '20%' } },
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(6, bftiResReportServiceBaseUrl, programTypeToggleStatus, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, programTypeList, params)
      if (result.success) {
