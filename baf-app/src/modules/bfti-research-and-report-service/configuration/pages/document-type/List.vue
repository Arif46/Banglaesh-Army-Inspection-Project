  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('configuration.document_type') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('configuration.document_type_s') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { documentTypeNameListApi, documentTypeNameToggleApi } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('configuration.document_type') + ' ' + this.$t('globalTrans.entry') : this.$t('configuration.document_type') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(6, bftiResReportServiceBaseUrl, documentTypeNameToggleApi, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, documentTypeNameListApi, params)
      if (result.success) {
