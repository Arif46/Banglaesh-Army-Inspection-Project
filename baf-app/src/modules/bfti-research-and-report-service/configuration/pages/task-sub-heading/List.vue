  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('configuration.task_sub_heading') }}</h4>
      </template>
      <template v-slot:searchBody>
        <b-row>
          ...
          <b-col xs="12" sm="12" md="4" lg="4" xl="4">
            ...
            >
              <template v-slot:label>
                {{ $t('configuration.task_heading') }}
              </template>
              <b-form-select
                plain
                id="task_heading_id"
                :options="taskHeadingList"
                v-model="search.task_heading_id"
              >
                <template v-slot:first>
                  <b-form-select-option :value="0">{{$t('globalTrans.select')}}</b-form-select-option>
                </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('configuration.task_sub_heading') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal id="details" size="lg" :title="$t('configuration.task_sub_heading') + ' ' + $t('globalTrans.details')"  hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { taskSubHeadingListApi, taskSubHeadingToggleApi } from '../../api/routes'
...
export default {
  ...
  'search.limit': function (newVal, oldVal) {
    ...
    'search.document_type_id': function (newVal, oldVal) {
      if (newVal !== oldVal) {
        this.taskHeadingList = this.getTaskHeadingList()
      }
    }
  },
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('configuration.task_sub_heading') + ' ' + this.$t('globalTrans.entry') : this.$t('configuration.task_sub_heading') + ' ' + this.$t('globalTrans.modify')
    },
    documentTypeList () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(6, bftiResReportServiceBaseUrl, taskSubHeadingToggleApi, item)
      } else if (type === 3) {
        this.details(item)
      }
    },
    ...
    getTaskHeadingList () {
      return this.$store.state.BftiResearchAndReportService.commonObj.taskHeadingList.filter(item => item.status === 1 && item.document_type_id === this.search.document_type_id).map(itemObj => {
        if (this.$i18n.locale === 'bn') {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, taskSubHeadingListApi, params)
      if (result.success) {
