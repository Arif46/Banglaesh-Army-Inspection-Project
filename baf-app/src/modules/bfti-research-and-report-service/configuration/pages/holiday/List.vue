  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('configuration.holiday') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('configuration.holiday') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  <template v-slot:cell(serial)="data">
                   ...
                   <template v-slot:cell(holiday_date)="data">
                      {{ data.item.holiday_date | dateFormat }}
                    </template>
...
import { holidayList, holidayToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.configuration.holiday_title, label_bn: i18n.messages.bn.configuration.holiday_title, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '30%' } },
  { label_en: i18n.messages.en.configuration.holiday_date, label_bn: i18n.messages.bn.configuration.holiday_date, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '15%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('configuration.holiday') + ' ' + this.$t('globalTrans.entry') : this.$t('configuration.holiday') + ' ' + this.$t('globalTrans.modify')
    },
    yearList () {
  ...
  methods: {
    remove (item) {
      this.changeStatus(6, bftiResReportServiceBaseUrl, holidayToggle, item, 'bfti-research-and-report-service')
    },
    searchData () {
      this.loadData()
    },
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, holidayList, params)
      if (result.success) {
