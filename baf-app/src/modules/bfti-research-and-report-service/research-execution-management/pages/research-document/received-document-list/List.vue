  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('research_execution.received_document') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('research_execution.received_document') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <b-button  title="Recommended" v-b-modal.modal-5 variant="iq-bg-info" class="action-btn btn-info" size="sm" @click="edit(data.item)"><i class="ri-share-forward-line"></i></b-button>
                    <b-button title="Approve" v-b-modal.modal-6 variant="iq-bg-info" class="action-btn btn-success" size="sm" @click="edit(data.item)"><i class="ri-check-line"></i></b-button>
                    <b-button  title="Reject" v-b-modal.modal-4 variant=" iq-bg-success border-info" class="action-btn btn-danger" size="sm" @click="edit(data.item)"><i class="ri-indeterminate-circle-line"></i></b-button>
    ...
    <b-modal id="modal-4" size="lg" :title="$t('globalTrans.reject')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <Reject :id="editItemId" :item="item"/>
    </b-modal>
    <b-modal id="modal-6" size="lg" :title="$t('globalTrans.approve')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <Approve :id="editItemId"  :item="item"/>
    </b-modal>
    <b-modal id="modal-7" size="lg" :title="$t('globalTrans.review')" :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger" hide-footer>
      <Review :id="editItemId" :item="item"/>
    </b-modal>
  </div>
</template>
<script>
import Detail from '../research-document-upload/Detail.vue'
import Forward from '../research-document-upload/Forward.vue'
import Approve from '../research-document-upload/Approve.vue'
import Reject from '../research-document-upload/Reject.vue'
import Review from '../research-document-upload/Review.vue'
import RestApi, { bftiResReportServiceBaseUrl, authServiceBaseUrl } from '@/config/api_config'
import { researchDocumentForwardedList, assignTaskCoordinatorToggle, userListByDesignWise } from '../../../api/routes'
import ModalBaseMasterList from '@/mixins/list'

export default {
  mixins: [ModalBaseMasterList],
  components: {
    Detail, Forward, Approve, Reject, Review
  },
  data () {
    return {
      sortBy: '',
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('research_execution.received_document') + ' ' + this.$t('globalTrans.entry') : this.$t('research_execution.received_document') + ' ' + this.$t('globalTrans.modify')
    },
    formNonAwarded () {
      return (this.editItemId === 0) ? this.$t('research_execution.received_document') + ' ' + this.$t('globalTrans.entry') : this.$t('research_execution.received_document') + ' ' + this.$t('globalTrans.modify')
    },
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, researchDocumentForwardedList, params)
      if (result.success) {
