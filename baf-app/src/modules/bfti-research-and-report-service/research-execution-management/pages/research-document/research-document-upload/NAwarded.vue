        ...
        <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off">
            ...
            <template v-slot:body>
             <b-row>
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                  <ValidationProvider name="Cancel Date" vid="non_awarded_date" rules="required" v-slot="{ valid, errors }">
                    <b-form-group
                      label-for="non_awarded_date"
                      label-cols-sm="4"
                    >
                      <template v-slot:label>
                        {{ $t('research_execution.non_awarded') +' '+$t('globalTrans.date') }}  <span class="text-danger">*</span>
                      </template>
                      <date-picker
                          class="form-control"
                          :placeholder="$t('globalTrans.select_date')"
                          id="non_awarded_date"
                          v-model="form.non_awarded_date"
                ...
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                <ValidationProvider name="Comment" vid="non_awarded_comment" rules="">
                    <b-form-group
                        class="row"
                        label-cols-sm="4"
                        label-for="non_awarded_comment"
                    ...
                    <b-form-textarea
                        id="non_awarded_comment"
                        v-model="form.non_awarded_comment"
                        :state="errors[0] ? false : (valid ? true : null)"
                    ></b-form-textarea>
          ...
          <b-row class="text-right">
            <b-col>
                ...
                <b-button variant="danger" class="btn-sm" @click="$bvModal.hide('modal-nonAwarded')">{{ $t('globalTrans.cancel') }}</b-button>
            </b-col>
          </b-row>
        </b-form>
      </b-overlay>
...
import { researchDocumentNonAwardDocument } from '../../../api/routes'
export default {
  name: 'NAwarded',
  props: ['id'],
  data () {
  ...
  methods: {
    async saveUpdate () {
      ...
      const result = await RestApi.putData(bftiResReportServiceBaseUrl, `${researchDocumentNonAwardDocument}/${this.id}`, this.form)
      ...
      if (result.success) {
        ...
        this.$bvModal.hide('modal-nonAwarded')
      } else {
        if (result.errors) {
