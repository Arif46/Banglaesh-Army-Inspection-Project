        ...
        <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off">
            ...
            <template v-slot:body>
             <b-row>
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                  <ValidationProvider name="Cancel Date" vid="awarded_date" rules="required" v-slot="{ valid, errors }">
                    <b-form-group
                      label-for="awarded_date"
                      label-cols-sm="4"
                    >
                      <template v-slot:label>
                        {{ $t('research_execution.awarded') +' '+$t('globalTrans.date') }}  <span class="text-danger">*</span>
                      </template>
                      <date-picker
                          id="awarded_date"
                          class="form-control"
                ...
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                <ValidationProvider name="Comment" vid="awarded_comment" rules="">
                    <b-form-group
                        class="row"
                        label-cols-sm="4"
                        label-for="awarded_comment"
                ...
                <b-col xs="12" sm="12" md="6" lg="6" xl="6">
                <ValidationProvider name="Attachment" vid="file" v-slot="{ valid, errors }" rules="">
                  <b-form-group
                    label-cols-sm="4"
                    label-for="file">
                    <template v-slot:label>
                ...
                <slot v-if="form.document_upload">
                  <template class="pb-3" v-if="form.document_upload.id && form.document_upload.awarded_file">
                    <a :href="bftiResReportServiceBaseUrl + (isImage(form.document_upload.submit_file) ? 'storage/' : '') + form.document_upload.awarded_file" target="_blank" class="btn btn-sm btn-success" download><i class="ri-file-download-fill"></i> {{ $t('bfti.attachment_down')}}</a> <br>
                  </template>
                </slot>
              </b-col>
              </b-row>
          ...
          <b-row class="text-right">
            <b-col>
                ...
                <b-button variant="danger" class="btn-sm" @click="$bvModal.hide('modal-awarded')">{{ $t('globalTrans.cancel') }}</b-button>
            </b-col>
          </b-row>
        </b-form>
      </b-overlay>
...
import { researchDocumentAwardDocument } from '../../../api/routes'
export default {
  name: 'Award',
  props: ['id'],
  data () {
  ...
  methods: {
    onChange (event) {
      ...
      if (input.files && input.files[0]) {
        ...
        reader.onload = (e) => {
          this.form.awarded_file = e.target.result
        }
        reader.readAsDataURL(input.files[0])
      } else {
        this.form.awarded_file = ''
      }
    },
    async saveUpdate () {
      ...
      const result = await RestApi.putData(bftiResReportServiceBaseUrl, `${researchDocumentAwardDocument}/${this.id}`, this.form)
      ...
      if (result.success) {
        ...
        this.$bvModal.hide('modal-awarded')
      } else {
        if (result.errors) {
