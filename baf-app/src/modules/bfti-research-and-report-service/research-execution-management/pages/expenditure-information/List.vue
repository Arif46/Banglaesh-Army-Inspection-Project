  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('research_execution.expenditure_information') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('research_execution.expenditure_information') }} {{ $t('globalTrans.list') }}</h4>
      </template>
            ...
            <b-col md="12" class="table-responsive">
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(expense_date)="data">
                    {{data.item.expense_date|dateFormat}}
                  </template>
                  <template v-slot:cell(tot_expense_amount)="data">
                    {{ $n(data.item.tot_expense_amount, { useGrouping: false }) }}
                  </template>
...
<script>
  ...
  import { expenditureInfoListApi, contractSignedLists } from '../../api/routes'
  ...
  const defaultColumn = [
    ...
    { label_en: i18n.messages.en.research_execution.expense_date, label_bn: i18n.messages.bn.research_execution.rcv_date, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '10%' } },
    { label_en: i18n.messages.en.research_execution.expense_amount, label_bn: i18n.messages.bn.research_execution.rcv_amount, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '10%' } },
    { label_en: i18n.messages.en.globalTrans.action, label_bn: i18n.messages.bn.globalTrans.action, class: 'text-center', show: '1', order: 7, thStyle: { width: '10%' } }
  ]
  export default {
    ...
    computed: {
      ...
      detailsTitle () {
        return this.$t('research_execution.expenditure_information') + ' ' + this.$t('globalTrans.details')
      },
      formTitle () {
        return this.$t('research_execution.expenditure_information')
      },
      pageOptions () {
    ...
    methods: {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(bftiResReportServiceBaseUrl, expenditureInfoListApi, params)
        if (result.success) {
