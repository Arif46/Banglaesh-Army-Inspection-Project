  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('report_management.income_expenditure_report') + ' ' + $t('globalTrans.list') }}</h4>
      </template>
    ...
    <b-modal id="details" size="xl" :title="$t('report_management.income_expenditure_report') + ' ' + $t('globalTrans.details')"  hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { incomeExpenditureListReportRoute } from '../../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, incomeExpenditureListReportRoute, params)
      if (result.success) {
        const listData = result.data.map((item, index) => {
          ...
          if (item.document_type_id === 2) {
            ...
            topicData.duration_bn = item.prepare_rfp.duration
          }
