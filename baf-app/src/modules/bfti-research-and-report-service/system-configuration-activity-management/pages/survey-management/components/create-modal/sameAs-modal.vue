            ...
            <b-row>
                ...
                <b-col lg="6" sm="6" md="6">
                    <ValidationProvider name="Grids" vid="grids">
                        ...
                        <b-form-checkbox-group
                            class="text-dark"
                            v-model="inputFields.same_fields"
                            :options="sameListGrid"
                            :state="errors[0] ? false : (valid ? true : null)"
                        ></b-form-checkbox-group>
                        <div class="invalid-feedback">
                            {{ errors[0] }}
...
export default {
    ...
    created () {
        ...
        if (this.formData) {
            this.formData.forEach(i => {
                if (i.field_type !== 'sameAs_button' && i.field_type !== 'addMore_button' && i.field_type !== 'h1' && i.field_type !== 'h2' && i.field_type !== 'h3' && i.field_type !== 'h4' && i.field_type !== 'h5' && i.field_type !== 'h6' && i.field_type !== 'paragraph' && i.field_type !== 'empty') {
                    this.inputListGrid.push({
                        text: this.$i18n.locale === 'en' ? i.label : i.label_bn, value: i.id
                    })
                    this.sameListGrid.push({
                        text: this.$i18n.locale === 'en' ? i.label : i.label_bn, value: i.id
                    })
                }
            })
    ...
    methods: {
        ...
        saveSettings: function () {
            ...
            this.$emit('sameAsField', tmp, 'modal-15')
        }
    }
}
</script>
