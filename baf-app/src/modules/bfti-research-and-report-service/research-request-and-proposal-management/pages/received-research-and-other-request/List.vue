  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('globalTrans.received') }} {{ $t('externalUser.research_and_other_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('globalTrans.received') }} {{ $t('externalUser.research_and_other_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    ...
                    <!-- <b-button v-b-modal.modal-approve variant="iq-bg-success mr-1" size="sm" @click="approval(data.item)" class="btn-primary action-btn edit" v-b-tooltip.hover :title="$t('globalTrans.edit')"><i class="ri-check-line"></i></b-button> -->
                  </template>
...
<script>
  ...
  import { bftiResearchRequestReceivedList, bftiResearchRequestApproveRejectApi } from '../../api/routes'
  ...
  const defaultColumn = [
    { label_en: i18n.messages.en.globalTrans.sl_no, label_bn: i18n.messages.bn.globalTrans.sl_no, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 1, thStyle: { width: '5%' } },
    ...
    { label_en: i18n.messages.en.bfti.topic, label_bn: i18n.messages.bn.bfti.topic, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '15%' } },
    ...
    { label_en: i18n.messages.en.globalTrans.action, label_bn: i18n.messages.bn.globalTrans.action, class: 'text-center', show: '1', order: 10, thStyle: { width: '20%' } }
  ]
  export default {
    ...
    methods: {
      ...
      async loadData () {
        ...
        this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
        const result = await RestApi.getData(bftiResReportServiceBaseUrl, bftiResearchRequestReceivedList, params)
        if (result.success) {
      ...
      confirmData (data) {
        ...
        research_request_id: data.id,
        organization: data.organization,
        organization_bn: data.organization_bn
      }
        let result = null
        result = RestApi.getData(bftiResReportServiceBaseUrl, bftiResearchRequestApproveRejectApi, params)
        if (result.success) {
          this.$toast.success({
            title: 'Success',
