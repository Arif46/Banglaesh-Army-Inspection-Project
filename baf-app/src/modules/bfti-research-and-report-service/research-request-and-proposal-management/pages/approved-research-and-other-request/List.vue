  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('globalTrans.approved') }} {{ $t('externalUser.research_and_other_request') }} {{ $t('globalTrans.search') }}</h4>
      </template>
        ...
        <b-row>
          ...
          <b-col xs="12" sm="12" md="6">
            ...
            >
              ...
              <b-form-select
                plain
                v-model="search.approve_reject_id"
                :options="approveRejectList"
                id="approve_reject_id"
              >
                <template v-slot:first>
                  <b-form-select-option :value=0>{{$t('globalTrans.select')}}</b-form-select-option>
                </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('globalTrans.approved') }} {{ $t('externalUser.research_and_other_request') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
<script>
  ...
  import { bftiResearchRequestApprovedList } from '../../api/routes'
  import ModalBaseMasterList from '@/mixins/list'
  import i18n from '@/i18n'
 /* import Form from './Form' */
  import Details from '../research-and-other-request/Details'
  ...
  export default {
    ...
    computed: {
      ...
      approveRejectList () {
        return this.approveRejectData.map(item => {
          if (this.$i18n.locale === 'bn') {
            const data = { value: item.value, text: item.text_bn }
            return Object.assign({}, item, data)
          } else {
            const data = { value: item.value, text: item.text }
            return Object.assign({}, item, data)
          }
        })
      },
