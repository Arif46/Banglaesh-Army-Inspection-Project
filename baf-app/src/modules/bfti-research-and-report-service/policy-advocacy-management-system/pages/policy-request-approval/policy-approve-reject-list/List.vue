  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('policy_advocacy.policy') + ' ' + $t('globalTrans.approved') + ' ' + $t('globalTrans.reject') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
<script>
  ...
  import { policyApproveRejectListApi } from '../../../api/routes'
  ...
  export default {
    ...
    data () {
      return {
        ...
        statusTypeList: [
          { value: 2, text: this.$i18n.locale === 'en' ? 'Approve' : 'অনুমোদন' },
          { value: 3, text: this.$i18n.locale === 'en' ? 'Reject' : 'বাতিল' }
        ],
        item: '',
        sortDesc: true,
        sortDirection: 'desc',
    ...
    methods: {
      ...
      async loadData () {
        ...
        const result = await RestApi.getData(bftiResReportServiceBaseUrl, policyApproveRejectListApi, params)
        if (result.success) {
