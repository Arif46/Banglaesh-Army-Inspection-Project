  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('task_activity_monitoring.checklist') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('task_activity_monitoring.checklist') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <router-link class="action-btn edit" v-if="data.item.approve_status === 1 || data.item.approve_status === 6" variant="iq-bg-success mr-1" title="Edit" :to="`/bfti-research-and-report-service/task-activity-monitoring/checklist/from/${data.item.id}`"><i class="ri-ball-pen-fill"></i></router-link>
                    <router-link v-if="data.item.task_assign === 2" class="action-btn active view" variant="iq-bg-info mr-1" title="Details" :to="`/bfti-research-and-report-service/task-activity-monitoring/checklist/detail/${data.item.id}`"><i class="ri-eye-fill"></i></router-link>
                    <b-button class="action-btn btn-success" v-if="data.item.task_assign === 2 && data.item.approve_status === 1" @click="remove(data.item)" title="Submit" v-b-modal.details size="sm"><i class="ri-arrow-right-circle-fill"></i></b-button>
                    <b-button  title="recommended" v-if="data.item.approve_status === 2 || data.item.approve_status === 6" v-b-modal.modal-5 variant="iq-bg-info" class="action-btn btn-info" size="sm" @click="recommended(data.item)"><i class="ri-share-forward-line"></i></b-button>
                  </template>
                </b-table>
              </slot>
...
import { coordinatorTask, assignTaskCoordinatorToggle } from '../../api/routes'
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'task_activity_monitoring.approve_status', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '20%' } },
        { labels: 'globalTrans.comments', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '20%' } },
