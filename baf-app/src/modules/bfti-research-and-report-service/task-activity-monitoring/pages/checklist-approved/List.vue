  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('globalTrans.approve') }} {{ $t('task_activity_monitoring.checklist') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('globalTrans.approve') }}  {{ $t('task_activity_monitoring.checklist') }}</h4>
      </template>
     ...
     <b-modal id="modal-details" size="lg" :title="$t('task_activity_monitoring.checklist') + ' ' + $t('globalTrans.details')" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
...
import { checkListApprovedList, assignTaskCoordinatorToggle, userListByDesignWise } from '../../api/routes'
import ModalBaseMasterList from '@/mixins/list'
import Details from '../checklist-approved/Details'
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, checkListApprovedList, params)
      if (result.success) {
