  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('task_activity_monitoring.task') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('task_activity_monitoring.task') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <router-link class="action-btn edit" variant=" iq-bg-success mr-1" title="Edit" :to="`/bfti-research-and-report-service/task-activity-monitoring/task/detail/${data.item.id}`"><i class="ri-eye-fill"></i></router-link>
                  </template>
                </b-table>
              </slot>
...
import { taskList, assignTaskCoordinatorToggle, userListByDesignWise } from '../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bftiResReportServiceBaseUrl, taskList, params)
      if (result.success) {
