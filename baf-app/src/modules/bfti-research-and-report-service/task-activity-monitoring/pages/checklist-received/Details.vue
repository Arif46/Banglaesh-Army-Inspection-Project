                ...
                <b-tr>
                  ...
                  <b-td>
                    ...
                    <slot v-else-if="condinatorInfo.document_type_id === 3">
                      {{ ($i18n.locale === 'bn') ? condinatorInfo.research.topic_bn : form.research.topic }}
                    </slot>
                ...
                <b-col>
                  <b-table-simple bordered style="width: 100%">
                    <b-tr>
                      ...
                      <b-th style="width:150px !important">{{ $t('task_activity_monitoring.task_name') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('task_activity_monitoring.sub_task_name') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('task_activity_monitoring.note') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('globalTrans.start_date') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('globalTrans.end_date') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('globalTrans.designation') }}</b-th>
                      <b-th style="width:150px !important">{{ $t('research_plan_management.responsible_person') }}</b-th>
                    </b-tr>
    ...
    async loadData () {
      ...
      if (result.success) {
        ...
        this.taskHeading = result.taskHeading.map((item, index) => {
          let isTaskBefore = []
          if (item.task_heading_id) {
            isTaskBefore = task.find(task => task.task_heading_id === item.id && task.sub_task_heading_id === parseInt(item.task_heading_id))
          } else {
            isTaskBefore = task.find(task => task.task_heading_id === item.id)
          }
          let taskHistory = [
            {
              id: 0,
