...
import { faqStoreApi, faqUpdateApi } from '../../api/routes'
export default {
  name: 'Form',
  props: ['id'],
  data () {
  ...
  methods: {
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(commonServiceBaseUrl, `${faqUpdateApi}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(commonServiceBaseUrl, faqStoreApi, this.form)
      }
