        ...
        <b-overlay :show="load">
          <ValidationObserver ref="form"  v-slot="{ handleSubmit, reset }">
            <b-form  @submit.prevent="handleSubmit(register)" @reset.prevent="reset" enctype="multipart/form-data">
              ...
              <ValidationProvider :name="$t('moc_portal.title_News_bn')" vid="title" rules="required">
                <b-form-group
                  class="row"
                  label-cols-sm="3"
                  label-for="title"
...
import { NewsStoreApi, NewsUpdateApi } from '../../api/routes'
  ...
  methods: {
    ...
    async register () {
      ...
      if (this.form.id) {
        ...
        result = await RestApi.putData(commonServiceBaseUrl, `${NewsUpdateApi}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(commonServiceBaseUrl, NewsStoreApi, this.form)
      }
      this.load = false
