  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('moc_portal.notice') }}</h4>
      </template>
...
import { noticeListApi, noticeToggleStatusApi } from '../../api/routes'
import ModalBaseMasterList from '@/mixins/list'

const defaultColumn = [
  { labels: 'globalTrans.sl_no', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 1, thStyle: { width: '8%' } },
  { labels: 'moc_portal.notice', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '40%' } },
  { labels: 'moc_portal.description1', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '50%' } },
  { labels: 'globalTrans.attachment', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '10%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('moc_portal.notice') + ' ' + this.$t('globalTrans.entry') : this.$t('moc_portal.notice') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    searchData () {
     ...
     async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, noticeListApi, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, noticeToggleStatusApi, item)
    }
  }
}
</script>
