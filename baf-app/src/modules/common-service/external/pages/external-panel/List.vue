  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('external.external_panel') }} {{ $t('globalTrans.list') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('external.external_panel') }} {{ $t('globalTrans.search') }}</h4>
      </template>
...
import { externalPanelList, externalPanelToggle } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      orderChange: {
        checkList: this.$store.state.CommonService.commonObj.externalPanelList,
        type: 'Comp',
        colspan: 3,
        urlSave: 'external/change-serial-order/ExternalPanel'
      }
    }
  },
  created () {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('external.external_panel') + ' ' + this.$t('globalTrans.entry') : this.$t('external.external_panel') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, commonServiceBaseUrl, externalPanelToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, externalPanelList, params)
      if (result.success) {
