  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('orgProfile.thana') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('orgProfile.thana') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { thanaList, thanaToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.orgProfile.thana, label_bn: i18n.messages.bn.orgProfile.thana, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '35%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('orgProfile.thana') + ' ' + this.$t('globalTrans.entry') : this.$t('orgProfile.thana') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, thanaList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, thanaToggle, item)
    }
  }
}
</script>
