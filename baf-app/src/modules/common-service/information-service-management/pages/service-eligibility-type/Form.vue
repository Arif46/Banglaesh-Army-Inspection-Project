...
import { serviceEligibilityTypeStoreApi, serviceEligibilityTypeUpdateApi } from '../../api/routes'
export default {
  name: 'Form',
  props: ['id'],
  data () {
  ...
  methods: {
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(commonServiceBaseUrl, `${serviceEligibilityTypeUpdateApi}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(commonServiceBaseUrl, serviceEligibilityTypeStoreApi, this.form)
      }
