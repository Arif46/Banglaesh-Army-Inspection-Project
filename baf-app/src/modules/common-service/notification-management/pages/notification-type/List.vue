  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('notification.notification_type') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('notification.notification_type') + ' ' + $t('globalTrans.list')}}</h4>
      </template>
...
import { notificationTypeList, notificationTypeToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.notification.notification_type, label_bn: i18n.messages.bn.notification.notification_type, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '70%' } },
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, notificationTypeList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(1, commonServiceBaseUrl, notificationTypeToggle, item)
    }
  }
}
</script>
