  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('committee.committee') }} {{ $t('globalTrans.search') }}</h4>
      </template>
      <template v-slot:searchBody>
         <b-row>
            ...
            <b-col xs="12" sm="12" md="4" lg="4" xl="4">
                <ValidationProvider name="Organization" vid="committee_name">
                    <b-form-group
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('committee.committee') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { committeeList, committeeToggle } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'committee.purposel', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '10%' } },
        { labels: 'committee.committee_duration_with_day', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '10%' } },
        { labels: 'committee.document', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '10%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('committee.committee_entey') : this.$t('committee.committee_modify')
    }
  },
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, commonServiceBaseUrl, committeeToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, committeeList, params)
      if (result.success) {
        const listData = result.data.data.map((item, index) => {
          const orgObj = this.$store.state.CommonService.commonObj.orgProfileList.find(division => division.value === parseInt(item.org_id))
          const orgData = {}
    ...
    remove (item) {
      this.changeStatus(commonServiceBaseUrl, committeeToggle, item)
    }
  }
}
</script>
