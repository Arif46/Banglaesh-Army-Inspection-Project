  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('complain_management.complain_chain') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('complain_management.complain_chain') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { complainChainList, complainChainToggle } from '../../api/routes'
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('complain_management.complain_chain') + ' ' + this.$t('globalTrans.entry') : this.$t('complain_management.complain_chain') + ' ' + this.$t('globalTrans.modify')
    },
    orgProfileList () {
  ...
  methods: {
    ...
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(1, commonServiceBaseUrl, complainChainToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(commonServiceBaseUrl, complainChainList, params)
      if (result.success) {
