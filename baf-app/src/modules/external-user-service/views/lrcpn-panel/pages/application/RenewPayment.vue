            ...
            <table v-if="history.length" class="table table-bordered">
                  <tr>
                    ...
                    <tr v-for="(item, index) in history" :key="index">
                        <td>{{ $t('externalLrcpn.renew_fee') }}</td>
                        <td>{{ $n(item.amount) }}</td>
                        <td>
           ...
           <b-row v-else>
            ...
            <div class="row mt-3">
                <div class="col-md-12 text-right">
                    <b-button variant="danger" class="mr-1 mt-3" @click="$bvModal.hide('modal-payment-renew')">{{ $t('globalTrans.cancel') }}</b-button>
                </div>
            </div>
        </b-col>
    </b-overlay>
...
export default {
  ...
  methods: {
      ...
      async saveFormData (type, transId = '') {
          ...
          this.$bvModal.hide('modal-payment-renew')
          if (result.success) {
            if (result.success === 2) {
              this.$toast.success({
                title: 'Success',
