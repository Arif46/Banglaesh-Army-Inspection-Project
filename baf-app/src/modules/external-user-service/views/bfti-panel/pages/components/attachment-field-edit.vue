                ...
                <ValidationProvider :name="data.label" :vid="data.field_name" :rules="data.isRequired === true ? `required` : ``">
                    ...
                    >
                        ...
                        <button type="button" v-if="getFile(data)" class="btn btn-sm btn-success">
                            <a target="_blank" v-if="formSubmit" :href="imgData" download="uploaded_file"><span style="margin: 15px 1px 15px !important; font-size: 12px;color:white !important">{{ $t('externalLrcpn.view_attachment') }}</span></a>
                            <a target="_blank" v-else :href="bftiBaseUrl + 'uploads/bfti/' + imgData"><span style="margin: 15px 1px 15px !important; font-size: 12px;color:white !important">{{ $t('externalLrcpn.view_attachment') }}</span></a>
                        </button>
        ...
        if (typeof (this.data.value[0]) !== 'undefined' && this.data.field_type !== 'image') {
            ...
            if (typeof (this.data.value[0]) === 'undefined') {
                ...
                if (checkJson) {
                    ...
                    const fileData = JSON.parse(this.data.value)
                    this.imgData = fileData[0]
                    this.initialImage = this.bftiBaseUrl + 'uploads/bfti/' + this.imgData
                } else {
