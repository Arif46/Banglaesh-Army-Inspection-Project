<template>
    <b-col md="12" sm="12" lg="12" class="text-center">
        ...
        <div class="custom-wrap" v-if="data.field_type === 'file'">
            <div class="custom-label" v-if="!reportView">{{ $i18n.locale === 'bn' ? data.label_bn : data.label }} : </div>
            <div class="custom-data">
                <button type="button" v-if="getFile(data.value)" class="btn btn-sm btn-success">
                    <a target="_blank" :href="bftiBaseUrl + 'uploads/bfti/' + uploadedfile"><i style="margin: 15px 1px 15px !important; font-size: 12px;color:white !important" class="ri-download-line"></i></a>
                </button>
