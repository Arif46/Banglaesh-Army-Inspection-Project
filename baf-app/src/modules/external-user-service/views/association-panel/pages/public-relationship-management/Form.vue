        ...
        <b-row>
          ...
          <b-col xs="6" sm="6" md="6">
              ...
              <b-form-group
                  label-for="url">
                  <template v-slot:label>
                    {{ $t('globalTrans.youtube') }} {{ $t('globalTrans.share') }} {{ $t('globalTrans.url') }} <span class="text-danger">*</span>
                  </template>
                  <b-form-input
                      id="url"
                      v-model="form.url"
...
import { publicRelationAdvertisementStore, publicRelationAdvertisementUpdate } from '../../api/routes'
  ...
  methods: {
    ...
    async saveUpdate () {
      ...
      if (isValid) {
          ...
          if (this.id) {
            result = await RestApi.putData(eBizServiceBaseUrl, `${publicRelationAdvertisementUpdate}/${this.id}`, this.form)
          } else {
            result = await RestApi.postData(eBizServiceBaseUrl, publicRelationAdvertisementStore, this.form)
          }
