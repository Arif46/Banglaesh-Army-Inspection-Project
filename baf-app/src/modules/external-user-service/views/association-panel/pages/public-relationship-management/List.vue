  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('eBizPromotional.public_relationship_management') }} {{$t('globalTrans.search')}}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('eBizPromotional.public_relationship_management') }}</h4>
      </template>
              ...
              <slot v-if="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <b-button v-if="data.item.draft_status < 2" v-b-modal.modal-form variant="iq-bg-success mr-1" size="sm" @click="edit(data.item)" class="action-btn edit" :title="$t('globalTrans.edit')"><i class="ri-pencil-fill"></i></b-button>
...
import { publicRelationAdvertisementList } from '../../api/routes'
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'globalTrans.expiry_title', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4, thStyle: { width: '15%' } },
        { labels: 'globalTrans.status', class: 'text-left', sortable: true, stickyColumn: false, show: '1', order: 5, thStyle: { width: '10%' } },
        { labels: 'globalTrans.action', class: 'text-left', sortable: true, stickyColumn: false, show: '1', order: 6, thStyle: { width: '5%' } }
      ],
      actions: {
        edit: false,
        details: true,
  ...
  methods: {
    ...
    async loadData () {
      if (this.$store.state.ExternalUserService.eBizPanel.associationProfile) {
        this.search.council_info_id = this.$store.state.ExternalUserService.eBizPanel.associationProfile.council_info_id
        this.search.association_info_id = this.$store.state.ExternalUserService.eBizPanel.associationProfile.association_info_id
      }
      const params = Object.assign({}, this.search, { page: this.pagination.currentPage, per_page: this.pagination.perPage })
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getData(eBizServiceBaseUrl, publicRelationAdvertisementList, params)
      if (result.success) {
