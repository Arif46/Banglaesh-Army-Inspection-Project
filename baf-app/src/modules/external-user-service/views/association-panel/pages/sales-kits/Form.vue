      ...
      <b-form @submit.prevent="handleSubmit(saveUpdate)" @reset.prevent="reset" autocomplete="off"  enctype="multipart/form-data">
        <b-row>
         ...
         <!-- <b-col xs="12" sm="12" md="6">
            <ValidationProvider name="Processed Area" vid="product_id" v-slot="{ errors }">
                <b-form-group
                    label-for="product_id">
                    <template v-slot:label>
                        {{ $t('eBizConfig.product_info') }}
                    </template>
                    <v-select
                        id="product_id"
          ...
          <b-col xs="12" sm="12" md="6">
            <ValidationProvider name="Product" vid="product_id" rules="required|min_value:1" v-slot="{ errors }">
              <b-form-group
                label-for="product_id"
              >
                <template v-slot:label>
                  {{ $t('eBizConfig.product_info') }} <span class="text-danger">*</span>
                </template>
                <b-form-select
                  plain
                  id="product_id"
...
import { salesKitsAdvertisementStore, salesKitsAdvertisementUpdate, opdPromotionalPortalList } from '../../api/routes'
...
export default {
  ...
  created () {
    ...
    if (this.id) {
      ...
      this.form = tmp
      if (tmp.start_date === '0000-00-00') {
         this.form.start_date = ''
      }
      if (tmp.end_date === '0000-00-00') {
         this.form.end_date = ''
      }
  ...
  methods: {
    async getProductList () {
      await RestApi.getData(eBizServiceBaseUrl, opdPromotionalPortalList).then(response => {
          if (this.$store.state.ExternalUserService.eBizPanel.associationProfile) {
            this.productList = response.data.filter(item => item.council_info_id === parseInt(this.$store.state.ExternalUserService.eBizPanel.associationProfile.council_info_id))
          } else {
    ...
    async saveUpdate () {
      ...
      if (isValid) {
          ...
          if (this.id) {
            result = await RestApi.putData(eBizServiceBaseUrl, `${salesKitsAdvertisementUpdate}/${this.id}`, this.form)
          } else {
            result = await RestApi.postData(eBizServiceBaseUrl, salesKitsAdvertisementStore, this.form)
          }
