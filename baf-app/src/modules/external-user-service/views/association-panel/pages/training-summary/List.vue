<template>
  <div class="inner-section">
                ...
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                 ...
                 <template v-slot:cell(program_image)="data">
                  ...
                  <template v-slot:cell(status)="data">
                    <span class="badge badge-warning" v-if="data.item.summary == null ">{{$t('globalTrans.pending')}}</span>
                    <span class="badge badge-warning" v-if="data.item.summary?.status === 1 ">{{$t('globalTrans.draft')}}</span>
                    <span class="badge badge-success" v-if="data.item.summary?.status === 2">{{$t('globalTrans.submitted')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    <b-button v-if="data.item.summary != null && data.item.summary.status == 1" v-b-modal.modal-form variant="iq-bg-success mr-1" size="sm" @click="editSummary(data.item)" class="action-btn edit" :title="$t('InformationTraining.training_summary') + ' ' + $t('globalTrans.edit')"><i class="ri-pencil-fill"></i></b-button>
                    <b-button v-if="data.item.summary == null" v-b-modal.modal-form variant="iq-bg-success mr-1" size="sm" @click="addSummary(data.item)" class="action-btn btn-success" :title="$t('InformationTraining.training_summary') + ' ' + $t('globalTrans.add')"><i class="ri-menu-add-fill"></i></b-button>
                    <b-button v-if="data.item.summary != null" v-b-modal.modal-details variant=" iq-bg-success mr-1" size="sm" @click="details(data.item)" class="action-btn edit" :title="$t('InformationTraining.training_summary') + ' ' + $t('globalTrans.details')"><i class="ri-eye-fill"></i></b-button>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-form" size="lg" :title="formTitle" hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      <Form :trainingId="editItemId" :summary="summary" :key="editItemId"/>
    </b-modal>
