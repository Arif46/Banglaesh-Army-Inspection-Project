  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('teaGardenPanel.tea_garden_registration') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('teaGardenPanel.tea_garden_registration') }} {{ $t('globalTrans.list') }}</h4>
      </template>
      <template v-slot:headerAction>
        <router-link to="tea-garden-application-form" class="btn-add"  v-b-modal.modal-form><i class="ri-add-fill"></i> {{ $t('globalTrans.add_new') }}</router-link>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(application_type)="data">
                    {{ data.item.application ? getFeeTypeName(data.item.application.application_type) : '' }}
                  </template>
                  <template v-slot:cell(status)="data">
                    {{ data.item.application ? getStatus(data.item.application.status) : '' }}
                  </template>
                  <template v-slot:cell(action)="data">
                    <router-link :to="'tea-garden-application/'+data.item.id" variant="iq-bg-success mr-1" size="sm" class="action-btn status" :title="$t('globalTrans.view')"><i class="ri-eye-fill"></i></router-link>
                   <b-button v-if="data.item.application.status === 1" variant=" iq-bg-success" size="sm" @click="editData(data.item)" class="action-btn edit" :title="$t('globalTrans.edit')"><i class="ri-pencil-fill"></i></b-button>
<!--                    <b-button v-b-modal.modal-certificate variant=" iq-bg-success mr-1" size="sm" @click="details(data.item)" class="action-btn status btn-warning" :title="$t('globalTrans.certificate')"><i class="ri-file-line"></i></b-button>-->
                    <router-link :to="'tea-garden-application-certificate/'+data.item.id" variant="iq-bg-success mr-1" size="sm" class="action-btn status btn-warning" :title="$t('globalTrans.certificate')"><i class="ri-file-line"></i></router-link>
                    <router-link v-if="isWarningTime(data.item.application)" :to="'tea-garden-renew-application-form/'+data.item.id" variant="iq-bg-success mr-1" size="sm" class="action-btn status delete blinking" :title="$t('globalTrans.renew')"><i class="ri-edit-circle-fill"></i></router-link>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-certificate" size="xl"  hide-footer :ok-title="$t('globalTrans.close')" ok-only ok-variant="danger">
      ...
      <Certificate :id="editItemId" :key="editItemId" ref="certificate"/>
    </b-modal>
  </div>
</template>
<script>
...
import Certificate from '@/modules/tea-garden-service/btb/pages/certificates/TeaGardenApplication'
import { gardenInfoListApi } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.teaGardenConfig.garden_name, label_bn: i18n.messages.bn.teaGardenConfig.garden_name, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2 },
  { label_en: i18n.messages.en.approvalProcess.app_id, label_bn: i18n.messages.bn.approvalProcess.app_id, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3 },
  { label_en: i18n.messages.en.teaGardenPanel.application_date, label_bn: i18n.messages.bn.teaGardenPanel.application_date, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 4 },
  { label_en: i18n.messages.en.teaGardenPanel.application_type, label_bn: i18n.messages.bn.teaGardenPanel.application_type, class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5 },
...
export default {
  ...
  computed: {
    ...
    certificateTitle () {
      return this.$t('teaGardenPanel.tea_garden_registration') + ' ' + this.$t('globalTrans.certificate')
    },
    pageOptions () {
  ...
  methods: {
    editData (item) {
      if (item.application.application_type === 2) {
        this.$router.push(`/tea-garden-panel/tea-garden-renew-application-form/${item.application.parent.taggable.id}?id=${item.id}`)
      } else {
        this.$router.push(`/tea-garden-panel/tea-garden-application-form?id=${item.id}`)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(teaGardenServiceBaseUrl, gardenInfoListApi, params)
      if (result.success) {
    ...
    getStatus (status) {
      let statusText
      if (status === 1) {
        statusText = this.$t('globalTrans.draft')
      } else if (status === 2) {
        statusText = this.$t('globalTrans.pending')
      } else if (status === 3) {
        statusText = this.$t('globalTrans.forward')
      } else if (status === 4) {
        statusText = this.$t('teaGardenPanel.recommendation')
      } else if (status === 5) {
        statusText = this.$t('globalTrans.recommended')
      } else if (status === 6) {
        statusText = this.$t('globalTrans.approved')
      } else {
        statusText = this.$t('globalTrans.rejected')
      }
      return statusText
    },
    pdfExport () {
      this.$refs.certificate.pdfExport()
    },
    isWarningTime (application) {
      const toDate = new Date()
      const warningDate = new Date(application.warning_date)
      return (toDate >= warningDate && !application.child)
    }
  }
}
</script>
