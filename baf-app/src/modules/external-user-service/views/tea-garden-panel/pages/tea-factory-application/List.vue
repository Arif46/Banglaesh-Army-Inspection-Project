  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerAction>
        <router-link to="tea-factory-application-form" class="btn-add"  v-b-modal.modal-form><i class="ri-add-fill"></i> {{ $t('globalTrans.add_new') }}</router-link>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(action)="data">
                    <!-- payment  -->
                    <router-link class="moc-action-btn moc-payment-btn" :to="`payment/${data.item.application.id}?from=tea_garden_panel.tea_factory_application`" v-if="data.item.application.status === 1 && data.item.application.payment_status == 0 && data.item.application.service.has_payment" :title="$t('teaGardenBtriService.payment_for')">
                    ...
                    <router-link :to="`tea-factory-application-renew-form/${data.item.id}`" v-if="data.item.application.warning_date && data.item.application.warning_date <= today" variant="iq-bg-success mr-1" size="sm" class="action-btn delete blinking" :title="$t('globalTrans.renew')"><i class="ri-edit-circle-fill"></i></router-link>
                    <router-link :to="`tea-factory-application-view/${data.item.id}`" variant="iq-bg-success mr-1" size="sm" class="action-btn status" :title="$t('globalTrans.view')"><i class="ri-eye-fill"></i></router-link>
                    <router-link v-if="data.item.application && data.item.application.status === 6" :to="`tea-factory-certificate/${data.item.id}`" variant="iq-bg-success mr-1" size="sm" class="action-btn btn-warning" :title="$t('globalTrans.certificate')"><i class="ri-file-line"></i></router-link>
...
export default {
  ...
  methods: {
    ...
    editData (item) {
      this.$router.push(`/tea-garden-panel/tea-factory-application-form?id=${item.id}`)
    },
    renewData (item) {
      this.$router.push(`/tea-garden-panel/tea-factory-application-renew-form/${item.application.parent_id}?id=${item.id}`)
    },
    searchData () {
      this.loadData()
    },
    ...
    getFactoryTypeName (id) {
      const factoryType = this.$store.state.ExternalUserService.teaGarden.commonObj.masterFactoryTypeList.find(item => item.value === id)
