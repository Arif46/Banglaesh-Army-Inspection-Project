        ...
        <div class="col-md-12" id="moneyReceiptArea">
          <table v-if="receipt.length" class="table table-bordered table-sm">
            <tr>
              ...
              <td colspan="4">{{ $i18n.locale === 'bn' ? application.details.company_name_bn : application.details.company_name }}</td>
            </tr>
            <tr>
              <th colspan="3">{{  $t('globalTrans.name') }}</th>
              <td colspan="4">{{ $i18n.locale === 'bn' ? application.details.applicant_name_bn : application.details.applicant_name }}</td>
            </tr>
            <tr>
              <th colspan="3">{{  $t('globalTrans.application_id') }}</th>
              <td colspan="4">{{ $n(application.application_id, { useGrouping: false }) }}</td>
            </tr>
            <tr>
...
export default {
  ...
  methods: {
    ...
    getServiceName (serviceId) {
      const ServiceName = this.$store.state.ExternalUserService.dealerPanel.commonObj.serviceNamesList.find(item => item.value === serviceId)
    ...
    pdfExport () {
        ...
        Pdf.exportPdfDetails(licenseRegistrationServiceBaseUrl, '/configuration/report-heading/detail', 2, reportTitle, this.receipt, this, this.application, this.transaction_no)
    }
  }
}
</script>
