  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('externalTradeFair.exhibit_information') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('externalTradeFair.exhibit_information') }} {{ $t('globalTrans.list') }}</h4>
      </template>
              ...
              <slot v-if ="columns.filter(item => item.show === '1').length > 0">
                <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(status)="data">
                    ...
                    <span class="badge badge-success" v-else-if="data.item.status == 5">{{$t('globalTrans.resubmit')}}</span>
                  </template>
                  <template v-slot:cell(action)="data">
                    ...
                    <router-link v-if="data.item.flag === 1 || data.item.status === 4" :to="`/trade-fair-panel/exhibit-information-form?id=${data.item.id}&exb_circular_id=${data.item.id}`" variant=" iq-bg-success mr-1" size="sm" class="action-btn edit" :title="$t('globalTrans.edit')"><i class="ri-pencil-fill"></i></router-link>
                  </template>
                </b-table>
              </slot>
    ...
    <b-modal id="modal-4" size="lg" :title="$t('externalTradeFair.exhibit_information') + ' ' + $t('globalTrans.details')" hide-footer ok-only ok-variant="danger">
...
import { exhibitionInfoList } from '../../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'externalTradeFair.subject', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 5, thStyle: { width: '15%' } },
        { labels: 'externalTradeFair.country_city', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 6, thStyle: { width: '25%' } },
        { labels: 'globalTrans.status', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 7, thStyle: { width: '10%' } },
        { labels: 'globalTrans.action', class: 'text-left', show: '1', order: 8, thStyle: { width: '20%' } }
      ],
      sortBy: '',
      item: '',
      detailsItemId: '',
  ...
  computed: {
    ...
    formTitle () {
      return (this.detailsItemId === 0) ? this.$t('externalTradeFair.exhibit_information') + ' ' + this.$t('globalTrans.entry') : this.$t('externalTradeFair.exhibit_information') + ' ' + this.$t('globalTrans.modify')
    },
    currentLocale () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, exhibitionInfoList, params)
      if (result.success) {
