...
import { DitfCircularParticipationDetails } from '../../../api/routes'
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(internationalTradeFairServiceBaseUrl, DitfCircularParticipationDetails, params)
      this.loading = false
      if (result.success) {
    ...
    async pdfExport () {
      this.loading = true
      const params = Object.assign({ request_type: 'pdf', local: this.$i18n.locale, org_id: 2, id: this.id })
      this.$store.dispatch('mutateCommonProperties', { loading: true, listReload: false })
      const result = await RestApi.getPdfData(internationalTradeFairServiceBaseUrl, DitfCircularParticipationDetails, params)
