  ...
  <div class="inner-section">
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('bazarMonitoring.commodity_name') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { commodityNameList, commodityNameToggle } from '../../api/routes'
...
const defaultColumn = [
  ...
  { label_en: i18n.messages.en.bazarMonitoring.commodity_name, label_bn: i18n.messages.bn.bazarMonitoring.commodity_name, class: 'text-center', sortable: true, stickyColumn: true, show: '1', order: 3, thStyle: { width: '35%' } },
...
export default {
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('bazarMonitoring.commodity_name') + ' ' + this.$t('globalTrans.entry') : this.$t('bazarMonitoring.commodity_name') + ' ' + this.$t('globalTrans.modify')
    },
    pageOptions () {
  ...
  methods: {
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(bazarMonitoringServiceBaseUrl, commodityNameList, params)
      if (result.success) {
    ...
    remove (item) {
      this.changeStatus(3, bazarMonitoringServiceBaseUrl, commodityNameToggle, item, 'bazar_monitoring_service')
    }
  }
}
</script>
