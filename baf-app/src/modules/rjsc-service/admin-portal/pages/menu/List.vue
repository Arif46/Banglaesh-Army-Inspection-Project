  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('sideBar.menu') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('sideBar.menu') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'rjscAdminPortal.parent_menu', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '30%' } },
        { labels: 'globalTrans.sorting_order', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '20%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('sideBar.menu') + ' ' + this.$t('globalTrans.entry') : this.$t('sideBar.menu') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(11, rjscServiceBaseUrl, menuToggle, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(rjscServiceBaseUrl, menuList, params)
      if (result.success) {
        const listData = result.data.data.map((item, index) => {
          const parentMenu = this.menuList.find(el => el.value === parseInt(item.parent_id))
          const parentMenuObj = {
            parent_menu_name_en: typeof parentMenu !== 'undefined' ? parentMenu.text_en : '',
            parent_menu_name_bn: typeof parentMenu !== 'undefined' ? parentMenu.text_bn : ''
          }
