  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('rjscAdminPortal.faq') }} {{ $t('globalTrans.search') }}</h4>
      </template>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('rjscAdminPortal.faq') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { faqList, faqToggleApi } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'rjscAdminPortal.question', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '55%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('rjscAdminPortal.faq') + ' ' + this.$t('globalTrans.entry') : this.$t('rjscAdminPortal.faq') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
    detailsData (item) {
    ...
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(11, rjscServiceBaseUrl, faqToggleApi, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(rjscServiceBaseUrl, faqList, params)
      if (result.success) {
