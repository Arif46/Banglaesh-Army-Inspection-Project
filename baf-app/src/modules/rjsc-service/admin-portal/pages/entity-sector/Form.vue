...
import { entitySectorStore, entitySectorUpdate } from '../../api/routes'
export default {
  name: 'Form',
  props: ['id'],
  data () {
  ...
  methods: {
    async saveUpdate () {
      ...
      if (this.id) {
        result = await RestApi.putData(rjscServiceBaseUrl, `${entitySectorUpdate}/${this.id}`, this.form)
      } else {
        result = await RestApi.postData(rjscServiceBaseUrl, entitySectorStore, this.form)
      }
