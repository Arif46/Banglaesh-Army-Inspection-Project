  ...
  <div class="inner-section">
    <card>
                ...
                <ValidationProvider name="Word" vid="word" rules="" v-slot="{ errors }">
                  <b-form-group label-for="word">
                    <template v-slot:label>
                      {{ $t('rjscConfig.considerable_word') }}
                    </template>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('rjscConfig.considerable_word') }} {{ $t('globalTrans.search') }}</h4>
      </template>
      <!-- search section end -->
    </card>
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('rjscConfig.considerable_word') }} {{ $t('globalTrans.list') }}</h4>
      </template>
...
import { considerableWordList, considerableWordToggleApi } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'rjscConfig.considerable_word', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2, thStyle: { width: '20%' } },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('rjscConfig.considerable_word') + ' ' + this.$t('globalTrans.entry') : this.$t('rjscConfig.considerable_word') + ' ' + this.$t('globalTrans.modify')
    }
  },
  methods: {
    editMethodInParent (item, type) {
      if (type === 1) {
        ...
        this.changeStatus(11, rjscServiceBaseUrl, considerableWordToggleApi, item)
      }
    },
    typeList () {
      const list = [
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(rjscServiceBaseUrl, considerableWordList, params)
      if (result.success) {
