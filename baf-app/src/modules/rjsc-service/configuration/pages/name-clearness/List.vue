  ...
  <div class="inner-section">
    <card>
      ...
      <template v-slot:searchHeaderTitle>
        <h4 class="card-title">{{ $t('rjscConfig.name_clearness') }}</h4>
      </template>
      <template v-slot:searchBody>
        <b-row>
          ...
          <ValidationProvider name="Business Type" vid="business_type_id" rules="required|min_value:1">
              ...
              <b-form-select
                  plain
                  v-model="search.business_type_id"
    ...
    <body-card>
      ...
      <template v-slot:headerTitle>
        <h4 class="card-title">{{ $t('rjscConfig.name_clearness') }} {{ $t('globalTrans.list') }}</h4>
      </template>
      ...
      <template v-slot:body>
        <b-overlay :show="loadingState">
          <b-table :sort-by.sync="sortBy" :sort-desc.sync="sortDesc" :sort-direction="sortDirection" head-variant="primary" class="tg" :items="listData" :emptyText="$t('globalTrans.noDataFound')" show-empty bordered hover :fields="columns.filter(item => item.show === '1').sort((a, b) => { return a.order - b.order })">
                  ...
                  <template v-slot:cell(entity_type_id)="data">
                    {{ getEntityTypeName(data.item.entity_type_id) }}
                  </template>
                  <template v-slot:cell(business_type_id)="data">
                    {{ getBusinessTypeName(data.item.business_type_id) }}
                  </template>
...
import { nameClearnessList, nameClearnessToggleApi } from '../../api/routes'
...
export default {
  ...
  data () {
    return {
      ...
      labelData: [
        ...
        { labels: 'rjscConfig.business_type', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 2 },
        { labels: 'rjscAdminPortal.entity_type', class: 'text-left', sortable: true, stickyColumn: true, show: '1', order: 3 },
  ...
  computed: {
    ...
    formTitle () {
      return (this.editItemId === 0) ? this.$t('rjscConfig.name_clearness') + ' ' + this.$t('globalTrans.entry') : this.$t('rjscConfig.name_clearness') + ' ' + this.$t('globalTrans.modify')
    },
    businessTypeList (id) {
  ...
  methods: {
    ...
    editMethodInParent (item, type) {
      ...
      if (type === 1) {
        ...
        this.changeStatus(6, rjscServiceBaseUrl, nameClearnessToggleApi, item)
      }
    },
    searchData () {
      this.loadData()
    ...
    async loadData () {
      ...
      const result = await RestApi.getData(rjscServiceBaseUrl, nameClearnessList, params)
      if (result.success) {
    ...
    getEntityTypeName (id) {
      const data = this.$store.state.RjscService.commonObj.entityTypeList.find(item => item.value === id)
    ...
    getBusinessTypeName (id) {
      const data = this.$store.state.RjscService.commonObj.businessTypeList.find(item => item.value === id)
      if (typeof data !== 'undefined') {
          return this.$i18n.locale === 'bn' ? data.text_bn : data.text_en
      } else {
          return ''
