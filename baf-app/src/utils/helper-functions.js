    ...
    getYearList (config = {}) {
        ...
        for (let i = parseInt(currentYear); i >= parseInt((currentYear - yearSpan)); i--) {
            ...
            if(window.vm.$i18n.locale === 'bn') {
                years.push({ value: i, text: convertEnglishToBanglaNumber(i.toString()), text_en: i, text_bn: convertEnglishToBanglaNumber(i.toString()) })
            } else {
                years.push({ value: i, text: i, text_en: i, text_bn: convertEnglishToBanglaNumber(i.toString()) })
            }
        }
        return years
        /* eslint-enable */
