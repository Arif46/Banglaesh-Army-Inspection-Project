    ...
    <ValidationProvider :name="data.label" :vid="data.field_name" :rules="checkValidate ? `required` : ``">
        ...
        >
            <template v-slot:label>
                {{ data.label }} <span class="text-danger" v-if="data.isRequired === '1'">*</span><span class="ml-2" style="border:1px solid #66cc99;border-radius:50%;cursor:pointer" data-toggle="tooltip" data-placement="top" :title="data.help_text">?</span>
            </template>
            <b-form-input
                :id="data.field_name"
                v-model="data.value"
