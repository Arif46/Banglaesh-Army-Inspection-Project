...
import tradeFairMixin from '@/mixins/trade-fair-service'
import commonServiceMixin from '@/mixins/common-service'
import { mapActions } from 'vuex'

export default {
  name: 'MainLayout',
  mixins: [tradeFairMixin, commonServiceMixin],
  data () {
    return {
      closeSidebar: false,
      mixinComponentId: 9
  ...
  watch: {
    currentLocale: function (newVal, oldVal) {
      if (newVal !== oldVal) {
        this.localizeCommonDropdownTradeFair({ value: newVal })
  ...
  methods: {
    ...
    ...mapActions({
      localizeCommonDropdownTradeFair: 'TradeFairService/localizeDropdown',
      // Common service Dropdown language Call function
