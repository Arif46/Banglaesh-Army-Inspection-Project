...
class CommitteeAgendaController extends Controller
{
    ...
    public function __construct(CommitteeAgenda $cmt_agenda)
    {
        // Set the model
        $this->model = new Repository($cmt_agenda);
    }
    ...
    */
    public function store(Request $request)
    {
        ...
        if ($request->attachment) {
            $image = Base64FileUpload::uploadFile($request, 'attachment', 'committee/cmtAgenda', null);
        ...
        if ($data['success']) {
            Cache::store('redis')->forget('agenda');
            App::make('agendaList');
        }
