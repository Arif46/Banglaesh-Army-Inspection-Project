...
class GenerateCirculateController extends Controller
{
    // space that we can use the repository from
    protected $model;

    public function __construct(CommitteeMetting $cmt_meeting)
    {
        // Set the model
        $this->model = new Repository($cmt_meeting);
    }
    ...
    */
    public function store(Request $request)
    {
        ...
        if ($request->document_attachment) {
            $image = Base64FileUpload::uploadFile($request, 'document_attachment', 'committee/cmtGenCirculate', null);
            $requestAll['document_attachment'] = $image['success'] ? $image['data']: null;
        }

        if ($request->attendance_attachment) {
            $image2 = Base64FileUpload::uploadFile($request, 'attendance_attachment', 'committee/cmtGenCirculate', null);
            $requestAll['attendance_attachment'] = $image2['success'] ? $image2['data']: null;
        }
